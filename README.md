...
独立开发者的 Cloudflare 单页实战手册。

从免费额度、能力清单、推荐栈、开源项目到上线检查，按一个页面组织，方便阅读、搜索和贡献。
...
- [学习路线](#学习路线)
- [能力清单](#能力清单)
- [Awesome Cloudflare Projects](#awesome-cloudflare-projects)
- [官方资源](#官方资源)
- [贡献方式](#贡献方式)
...
| 场景 | 推荐组合 | 说明 |
| --- | --- | --- |
| 博客 / 文档 / CMS | Pages、Workers、D1、R2 | 静态入口优先，动态管理和数据再交给 Workers / D1 / R2。 |
| 图床 / 文件 / R2 | Workers、R2、D1 | R2 放对象，D1 放元数据和权限。 |
| 邮箱 / 验证码 | Email Routing、D1、R2、Agent | 邮件入口、验证码提取、附件存储和 AI 处理可以组合起来。 |
| 短链 / API / Webhook | Workers、KV、D1 | Workers 做入口，KV 或 D1 存状态。 |
| 监控 / 分析 | Cron、Workers、D1、通知渠道 | 定时检查、保存状态、发通知。 |
| AI / Agent / RAG | Workers AI、AI Gateway、Vectorize、Durable Objects | 模型调用、网关观测、向量检索和会话状态分层处理。 |
