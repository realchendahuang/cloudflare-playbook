...
这一节不做全量 awesome list，只收录值得学习、能部署、架构有参考价值的项目。`awesome-cloudflare` 这类仓库是发现入口，不直接当成项目推荐；真正有价值的是从列表里筛出来的具体项目。

### 筛选标准

高质量项目至少满足几条：

- 真实使用 Cloudflare 能力，而不是只把静态页面部署到 Pages。
- README 说清楚部署路径，最好有 `wrangler`、Pages、D1、R2、KV、Durable Objects、Queues 等配置示例。
- 最近仍有维护，或项目本身足够稳定、社区使用面足够大。
- 同类项目里有明显架构价值，比如鉴权、存储设计、后台任务、日志、迁移、成本控制。
- 安全敏感项目要说明权限、加密、备份或访问控制，不只给一个能跑的 demo。

暂不收录这些：

- 只改 UI、改域名、改文案的同质 fork。
- archived 或 README 明确不再维护的项目，除非它是历史生态参考。
- 主要用途是代理、镜像、规避限制的项目，不作为学习 Cloudflare 的主线。
- 没有部署文档、没有配置文件、没有 Cloudflare 绑定说明的项目。
- 安全、邮箱、密码类项目如果威胁模型不清楚，只能源码参考，不能默认生产可用。

### 发现入口

- [zhuima/awesome-cloudflare](https://github.com/zhuima/awesome-cloudflare)：中文入口，项目多，适合继续挖具体仓库。
- [irazasyed/awesome-cloudflare](https://github.com/irazasyed/awesome-cloudflare)：英文综合清单，偏 Workers recipes、教程和资源。
- [ghostwriternr/awesome-cloudflare](https://github.com/ghostwriternr/awesome-cloudflare)：Developer Platform 入口，覆盖 Workers、D1、R2、Pages、AI 等方向。
- [lukeed/awesome-cloudflare-workers](https://github.com/lukeed/awesome-cloudflare-workers)：Workers 早期生态清单，适合查历史项目和 recipes。
- [cloudflare-workers topic](https://github.com/topics/cloudflare-workers)、[cloudflare-pages topic](https://github.com/topics/cloudflare-pages)、[cloudflare-d1 topic](https://github.com/topics/cloudflare-d1)、[cloudflare-r2 topic](https://github.com/topics/cloudflare-r2)：按 GitHub topic 继续发现项目。

### 官方仓库和模板

**首选：[cloudflare/templates](https://github.com/cloudflare/templates)**。这是官方模板总库，可以用 `npm create cloudflare@latest` 或 Dashboard 创建项目。重点看 `d1-template`、`r2-explorer-template`、`durable-chat-template`、`llm-chat-app-template`、`react-router-hono-fullstack-template`、`saas-admin-template`、`workflows-starter-template`、`containers-template`。
...
- [cloudflare/moltworker](https://github.com/cloudflare/moltworker)：OpenClaw 跑在 Cloudflare Sandbox 的实验项目，适合看 Containers/Sandbox 和 AI assistant 怎么组合；它更偏实验，不适合作为普通项目起步模板。
...
**CMS 首选：[microfeed/microfeed](https://github.com/microfeed/microfeed)**。它是自托管轻量 CMS，用 Pages、R2、D1、Zero Trust 组织内容、媒体、RSS 和 JSON feed，适合看“内容系统怎么 Cloudflare 原生化”。

**博客首选：[openRin/Rin](https://github.com/openRin/Rin)**。它基于 Pages、Workers、D1、R2 做边缘原生博客，后台、图片、文章和部署路径都比较完整。
...
**网盘/文件分享首选：[ling-drag0n/CloudPaste](https://github.com/ling-drag0n/CloudPaste)**。它是 Cloudflare Workers + Workflows + D1 架构，支持文件管理、文本分享、WebDAV、多存储后端和预览能力。想做“自己的轻量网盘”，优先看它。

**R2 管理首选：[G4brym/R2-Explorer](https://github.com/G4brym/R2-Explorer)**。它把 R2 bucket 做成类似 Google Drive 的管理界面，适合只想管理 R2 文件的人；它不是完整网盘，更像 R2 控制台增强。

**图床首选：[MarSeventh/CloudFlare-ImgBed](https://github.com/MarSeventh/CloudFlare-ImgBed)**。它是基于 Cloudflare 的文件/图床方案，支持多存储通道，适合做公开图片和个人文件托管。
...
- [cf-pages/Telegraph-Image](https://github.com/cf-pages/Telegraph-Image)：Pages + Telegraph 的图片托管方案，社区使用多，但它更依赖 Telegraph，不是 R2 最佳范式。
...
**临时邮箱首选：[dreamhunter2333/cloudflare_temp_email](https://github.com/dreamhunter2333/cloudflare_temp_email)**。它用 Cloudflare 免费服务搭临时邮箱，D1 存数据，支持前后端、附件、IMAP/SMTP、Telegram Bot 等能力，社区使用面很大。

**完整邮箱服务首选：[maillab/cloud-mail](https://github.com/maillab/cloud-mail)**。它基于 Cloudflare 做响应式邮箱服务，支持邮件发送、附件收发、R2 存附件、Workers AI 识别验证码，适合研究“Cloudflare 邮箱产品化”。
...
**首选：[miantiao-me/Sink](https://github.com/miantiao-me/Sink)**。它是 100% 跑在 Cloudflare 上的短链接系统，带分析、控制台、过期、密码和安全提示页，适合当短链项目最佳实践。
...
**网站统计首选：[benvinegar/counterscale](https://github.com/benvinegar/counterscale)**。它是自托管 Web Analytics，主要依赖 Workers 和 Analytics Engine，适合替代轻量 Umami/Plausible 场景。

**Uptime 监控首选：[lyc8503/UptimeFlare](https://github.com/lyc8503/UptimeFlare)**。它是 Workers 驱动的 uptime monitoring 和状态页，已迁移到 D1，支持全球地理位置检查，部署路径清楚。
...
**多产品管理首选：[DataflareApp/dataflare](https://github.com/DataflareApp/dataflare)**。它覆盖 D1、R2、KV、R2 SQL、Analytics Engine，适合集中管理 Cloudflare 数据产品。

**D1 GUI 首选：[outerbase/studio](https://github.com/outerbase/studio)**。它支持 Cloudflare D1，并提供 Deploy to Cloudflare，适合需要浏览器数据库 GUI 的场景。
...
**SaaS 鉴权首选：[zpg6/better-auth-cloudflare](https://github.com/zpg6/better-auth-cloudflare)**。它把 Better Auth 和 Workers、D1、Hyperdrive、KV、R2、地理位置能力接起来，适合 Next.js、Hono 等 Cloudflare 全栈项目。

**密码服务参考：[shuaiplus/nodewarden](https://github.com/shuaiplus/nodewarden)**。它是跑在 Workers 上的 Bitwarden-compatible server，支持 R2 或 KV 附件。密码类项目要先看备份、访问控制和迁移策略，再考虑生产使用。
...
**Agent 首选：[cloudflare/agents](https://github.com/cloudflare/agents)**。官方项目，基于 Durable Objects 承载有状态 Agent，是学习 Cloudflare Agent 架构的第一入口。

**AI 应用生成器首选：[cloudflare/vibesdk](https://github.com/cloudflare/vibesdk)**。它是跑在 Cloudflare 全栈上的 AI web app generator，适合看 AI 编程平台、部署按钮和项目生成流程。
...
**API 框架首选：[honojs/hono](https://github.com/honojs/hono)**。它不是 Cloudflare 专属项目，但已经是 Workers API 生态的核心框架，适合做 REST API、Webhook、MCP Server 和 BFF。

**SaaS Starter 首选：[supermemoryai/cloudflare-saas-stack](https://github.com/supermemoryai/cloudflare-saas-stack)**。它把 Cloudflare D1、Pages、鉴权、样式、存储等打包成可部署 SaaS 骨架，适合做产品原型。
...
- 想做网盘或文件分享：先看 [CloudPaste](https://github.com/ling-drag0n/CloudPaste)。只想管理 R2 bucket，看 [R2-Explorer](https://github.com/G4brym/R2-Explorer)。只想做临时投递，看 [cf-drop](https://github.com/lyonbot/cf-drop)。
- 想做图床：重视完整文件托管看 [CloudFlare-ImgBed](https://github.com/MarSeventh/CloudFlare-ImgBed)，重视 R2 + 元数据看 [imgUU](https://github.com/yestool/imgUU) 或 [PixR2](https://github.com/WangQueXL/PixR2)。
- 想做邮箱：临时邮箱看 [cloudflare_temp_email](https://github.com/dreamhunter2333/cloudflare_temp_email)，完整邮箱服务看 [cloud-mail](https://github.com/maillab/cloud-mail)，邮件转通知看 [mail2telegram](https://github.com/TBXark/mail2telegram)。
- 想做短链：带分析和后台看 [Sink](https://github.com/miantiao-me/Sink)，最小 KV 短链看 [Url-Shorten-Worker](https://github.com/crazypeace/Url-Shorten-Worker)。
- 想做统计：Web Analytics 看 [counterscale](https://github.com/benvinegar/counterscale)，状态页看 [UptimeFlare](https://github.com/lyc8503/UptimeFlare)。
- 想做 D1 管理：多产品统一看 [Dataflare](https://github.com/DataflareApp/dataflare)，D1 GUI 看 [Outerbase Studio](https://github.com/outerbase/studio) 或 [d1-manager](https://github.com/JacobLinCool/d1-manager)。
- 想做 SaaS：先看 [cloudflare-saas-stack](https://github.com/supermemoryai/cloudflare-saas-stack)，后端 API 看 [backend-api-kit](https://github.com/supermemoryai/backend-api-kit)，鉴权看 [better-auth-cloudflare](https://github.com/zpg6/better-auth-cloudflare)。
- 想做 AI Agent：先看 [cloudflare/agents](https://github.com/cloudflare/agents)，再看 [vibesdk](https://github.com/cloudflare/vibesdk) 和 [llmkit](https://github.com/smigolsmigol/llmkit)。
