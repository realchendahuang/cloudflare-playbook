...
```mermaid
flowchart TD
  subgraph OUT["出站 Email Sending（Public Beta，需 Workers Paid）"]
    direction TB
    W["Workers Binding<br/>env.EMAIL.send()"] --> PIPE["Cloudflare 投递管线<br/>DKIM + ARC 签名 / SPF 对齐 / DMARC"]
    R["REST API<br/>POST /email/sending/send"] --> PIPE
    S["SMTP<br/>smtps://smtp.mx.cloudflare.net:465"] --> PIPE
  end
  PIPE --> NET["全球边缘网络<br/>托管 IP 声誉 + 退信重试"]
  NET --> RCPT["收件人邮箱"]

  subgraph IN["入站 Email Routing（GA，Free 可用）"]
    direction TB
