...
| 已完成复核 | AI 产品二次精读 | 已把 [AI 产品](/platform/ai/) 对齐 AI Gateway limits / spend limits / logging、Workers AI pricing / limits / prompt caching / Markdown Conversion、AI Search limits-pricing / how it works / search modes / data source / public endpoints / MCP / REST API migration、Vectorize pricing / limits / create indexes / metadata filtering、Agents concepts / limits / memory / long-running agents / durable fibers / observability，补齐二次精读结论、上线前问题、成本控制、托管 RAG 新旧边界、自定义 RAG 权限设计和 Agent 长期运行边界。 |
...
| 已完成复核 | 扩展计算与数据管道二次精读 | 已把 [扩展计算与数据管道](/platform/extended-compute-data/) 对齐 Hyperdrive concepts / private database via Workers VPC / supported databases / metrics、Workflows rules / sleeping / retrying / Workers API / Python DAG、Pipelines streams / sinks / legacy migration、R2 Data Catalog pricing / table maintenance、Containers lifecycle / scaling / outbound traffic / bindings / rollouts，补齐二次精读结论、上线前问题、运行时边界和 R2 Data Catalog 最新计费口径。 |
...
| 已完成复核 | 平台文案收敛 | 已把 [Cloudflare 产品大图谱](/platform/) 改为产品入口和选型路线，删掉长链路图与百科式堆叠；同步压缩 [免费与付费边界](/platform/free-paid/) 的核对过程、重复导读和本站内部技术栈说明，并继续收敛 [D1](/platform/d1/)、[Queues](/platform/queues/)、[R2](/platform/r2/) 与 [Durable Objects](/platform/durable-objects/) 的代码、配置和过细实现说明。 |
