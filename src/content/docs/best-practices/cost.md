...
成本控制不是把项目省到不能用，而是知道什么行为触发计费，并在架构、配置和代码里留下边界。先盯四件事：静态请求有没有误进 Worker，写接口有没有被刷，数据是否放对位置，AI 和日志有没有上限。

Cloudflare 账单要分成域名计划、Workers Paid、附加产品和按量计费四条线看。预算提醒、用量面板和发票规则见 [账单与预算](/platform/billing/)；完整数字见 [免费额度大全](/platform/free-paid/)。
