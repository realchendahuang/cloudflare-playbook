...
独立项目先用免费层验证真实需求：静态站走 Workers Static Assets / Pages，动态入口走 Workers，关系数据进 D1，文件进 R2，读多写少配置进 KV，公开写入口加 Turnstile / Rate Limiting。完整数字统一看 [免费额度大全](/platform/free-paid/)，这页不重复维护价格表。
...
Workers Paid 是动态能力和工程化能力的底座，不是 Cloudflare Pro，也不是预算封顶。只有 Worker 请求、CPU、日志、Cron、D1、KV、Queues、Durable Objects 或 Workers AI 已经进入真实生产路径时再考虑；只是静态文档站、官网、博客或作品集时不急。
