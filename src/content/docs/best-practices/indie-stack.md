...
独立开发者用 Cloudflare，核心不是把所有产品都用上，而是把域名入口、静态内容、动态接口、数据、文件、安全和观测放到正确位置。这样免费阶段能跑起来，第一次付费也能知道自己买的是请求、CPU、存储、日志还是 AI 用量。
...
独立项目先用免费层验证真实需求：静态站走 Workers Static Assets / Pages，动态入口走 Workers，关系数据进 D1，文件进 R2，读多写少配置进 KV，公开写入口加 Turnstile 和限流。完整数字统一看 [免费额度大全](/platform/free-paid/)，这页不重复维护价格表。
...
| 产品化 | Workers Paid、Workers Logs、AI Search / Vectorize、高级接口防护、指标分析、客户域名能力。 | 只为真实瓶颈和真实用户需求付复杂度。 |

## 付费判断

Workers Paid 是动态能力和工程化能力的底座，不是 Cloudflare Pro，也不是预算封顶。只有 Worker 请求、CPU、日志、定时任务、D1、KV、Queues、Durable Objects 或 Workers AI 已经进入真实生产路径时再考虑；只是静态文档站、官网、博客或作品集时不急。
...
具体额度统一看 [免费额度大全](/platform/free-paid/)；官方场景资料从 [Cloudflare 文档地图](/reference/cloudflare-docs-map/) 进入。
