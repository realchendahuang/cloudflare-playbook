...
| 语义搜索 | Vectorize | 需要自然语言检索、相似内容查找和元数据过滤。 |
| 指标分析 | Analytics Engine | 高基数事件、用量计费、服务指标和客户级使用数据。 |
...
| API | 认证、限流、请求体大小、幂等键、错误日志和请求格式。 |
| 文件上传 | 短期签名、大小限制、文件类型校验、R2 元数据、必要时异步扫描。 |
| AI 调用 | AI Gateway 限流、缓存、日志和模型隔离；不要把模型密钥放前端。 |
