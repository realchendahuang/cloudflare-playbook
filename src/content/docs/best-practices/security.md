...
```text
Cloudflare 安全边界
  ├─ 公开入口：DNS / SSL/TLS / CDN / DDoS
  ├─ 写接口：WAF / Rate Limiting / Turnstile / 身份校验
  ├─ 后台入口：Access / Tunnel / 源站不暴露公网
  ├─ API：API Shield / OpenAPI schema / JWT / mTLS
  ├─ 密钥：wrangler secret / Secrets Store / bindings
  └─ 观测：Security Insights / Security Events / Workers Logs
```
...
| 7 | 生产密钥走 `wrangler secret` 或 Secrets Store。 | 密钥不能进入仓库、前端包、日志和公开文档。 |
...
| 检查项 | 最小要求 | 适合升级到 |
| --- | --- | --- |
| DNS | Web 入口 Proxied；邮件、验证、第三方服务按需 DNS-only。 | DNS 记录多、多人维护时纳入 Terraform / Pulumi。 |
...
| 密钥 | 单 Worker 用 `wrangler secret`；多 Worker 共享再用 Secrets Store。 | Secrets Store RBAC、审计日志、AI Gateway BYOK。 |
...
Turnstile 的关键点是：**前端组件不等于保护完成**。token 最长 2048 字符、5 分钟有效、单次使用；服务端必须调用 Siteverify，失败就不要继续写库、发邮件或触发 AI。
...
async function verifyTurnstile(token: string, ip: string, env: Env) {
  // Turnstile token 必须在服务端验证，不能只信任前端组件。
  ...
  // Siteverify 返回 success 后，业务写入才继续执行。
  return response.json<{ success: boolean; "error-codes"?: string[] }>();
}
```
...
1. 设置 session identifier，例如 Authorization header 或 JWT claim。
2. 保存 Endpoint Management 里的关键 endpoints。
3. 有 OpenAPI schema 就上传，没有就让 Cloudflare 基于流量学习。
4. Schema validation 先用 `log`，看 Security Events 确认没有误伤后再切 `block`。
5. 高风险 API 再评估 JWT validation、mTLS、Sequence Analytics 和 per-session rate limit。
...
- 前端永远拿不到服务端密钥。
- 单 Worker 生产密钥优先用 `wrangler secret put`。
- 多 Worker、多团队、AI Gateway BYOK 或账号级复用，再用 Secrets Store。

Secrets Store 是账号级密钥中心，和 per-Worker Variables and Secrets 不同。绑定到 Worker 时，代码只通过 `env` 读取，不把密钥写进 `wrangler.jsonc`、Markdown、日志或异常响应。
...
本站是 Astro / Starlight 文档站，部署在 Workers Static Assets 上。当前安全重点是：

| 模块 | 当前边界 |
| --- | --- |
| 文档页面 | 静态资产请求免费且不限量，正常情况不进入 Worker 脚本。 |
| 评论 | Twikoo Cloudflare 作为成熟组件；写入量增加时接入 Turnstile、Rate Limiting 和管理入口保护。 |
| 搜索 | Pagefind 构建期搜索，不产生后端搜索请求。 |
| 未来 AI 搜索 | 先走 AI Gateway 和配额，再评估 AI Search / Vectorize。 |
| 管理入口 | 一旦出现后台，默认用 Access / Tunnel，不自写登录页。 |

## GitHub 开源参考

| 仓库 | 用法 |
| --- | --- |
| [cloudflare/cloudflare-docs WAF source](https://github.com/cloudflare/cloudflare-docs/tree/production/src/content/docs/waf) | 追踪 Custom Rules、Rate Limiting、Managed Rules 和 Security Events 文档变化。 |
| [cloudflare/cloudflare-docs Turnstile source](https://github.com/cloudflare/cloudflare-docs/tree/production/src/content/docs/turnstile) | 追踪 Siteverify、plans、analytics 和 troubleshooting。 |
| [cloudflare/cloudflare-docs API Shield source](https://github.com/cloudflare/cloudflare-docs/tree/production/src/content/docs/api-shield) | 追踪 Endpoint Management、schema validation、JWT、mTLS 和 sequence analytics。 |
| [cloudflare/cloudflare-docs Cloudflare One source](https://github.com/cloudflare/cloudflare-docs/tree/production/src/content/docs/cloudflare-one) | 追踪 Access、Tunnel、Gateway 和 Zero Trust 计划边界。 |
| [cloudflare/cloudflared](https://github.com/cloudflare/cloudflared) | 理解 Tunnel 客户端、发布应用和源站连接模型。 |
| [cloudflare/terraform-provider-cloudflare](https://github.com/cloudflare/terraform-provider-cloudflare) | 把 DNS、WAF、Access、Rules 和账号资源纳入 review。 |
| [cloudflare/skills WAF reference](https://github.com/cloudflare/skills/tree/main/skills/cloudflare/references/waf) | 对照官方 Agent Skill 里的 WAF 配置模式和常见误区。 |
