...
最佳实践不是把 Cloudflare 产品尽量用满，而是先把免费额度、安全边界和升级信号讲清楚。普通项目最容易出问题的地方，是静态请求误进 Worker、写接口没有限流、文件放错位置、日志和 AI 没有预算边界。
...
| 给文档站、教程站或开源站选技术栈 | [本站技术栈](/best-practices/site-stack/) |
...
| 阶段 | 先做什么 | 不要急着做 |
| --- | --- | --- |
| 接入域名 | DNS 托管、Web 记录 Proxied、SSL/TLS Full (strict)、源站保护。 | 复制企业网络架构。 |
...
| 产品化 | 再看 Workers Paid、Workers Logs、Access、API Shield、Analytics Engine、AI Search 或 Vectorize。 | 在没有真实压力前购买复杂度。 |
...
| 数据性能 | D1 高频查询建索引；R2 浏览器访问配置权限和生命周期。 | [数据产品](/platform/data/)、[D1](/platform/d1/)、[R2](/platform/r2/) |
