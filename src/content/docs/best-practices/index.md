...
完整额度表见 [免费与付费边界](/platform/free-paid/)。写预算、报价单或 README 时，不要只引用二手整理，最终要回到官方 pricing / limits 页面核对。

## 读者入口

| 你要解决的问题 | 先读什么 | 读完要能判断什么 |
| --- | --- | --- |
| 不知道一个项目能不能 0 元跑起来 | [免费与付费边界](/platform/free-paid/) | Free、Workers Paid、zone plan、add-on 和 usage-based 的边界。 |
...
| 用 Codex 长期维护 Cloudflare 项目 | [Codex 协作](/best-practices/codex-cloudflare/) | 什么时候查官方 Markdown / `llms.txt`，什么时候看 GitHub 源仓库，什么时候用 Wrangler 验证。 |
...
| 密钥泄露 | 生产密钥使用 Secrets，不进源码、配置、日志和前端包。 | [Codex 协作](/best-practices/codex-cloudflare/)、[安全边界](/best-practices/security/) |
