...
文档站、开源教程站和轻社区先用成熟组合：框架选 Astro + Starlight，搜索先用 Pagefind，评论用 Twikoo Cloudflare，部署放到 Cloudflare 静态资产层。
...
主线是：**让读文档这条路径保持静态，让评论、搜索接口和后续接口走明确的动态边界。**

## 框架判断

VuePress 可以部署到 Cloudflare，也适合写普通教程。Cloudflare Playbook 这类内容站更适合 Astro + Starlight，因为主题、搜索、导航和静态输出都更省心。
...
早期文档站最重要的是目录、标题、交叉链接和可靠判断；向量搜索放到自然语言搜索需求明确后。

## 评论路线

评论区优先复用成熟组件，除非评论本身就是你的产品主线。
...
| 做法 | 边界 |
| --- | --- |
| 为文档站重写设计系统 | 文档站最值钱的是内容、判断和阅读体验；主题只做必要品牌收敛。 |
