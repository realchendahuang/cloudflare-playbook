...
- 后续如果加评论、AI Search、R2 上传、D1 示例和 API 示例，Workers 模型比纯文档生成器更贴合本站定位。

## 部署判断

Cloudflare Pages 仍然是纯静态站、博客、预览环境和 Git 部署的好选择。这里优先选择 Workers Static Assets，是因为它更方便把静态内容、评论、搜索增强、数据和未来 API 放进同一套 Workers 模型里。
