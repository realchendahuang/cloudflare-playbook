...
这篇给文档站、开源教程站和轻社区一个可复用选择：框架用成熟方案，搜索先静态，评论用成熟组件，部署放到 Cloudflare 静态资产层，动态能力只在必要路径出现。
...
| 自然语言搜索 | AI Search / AI Gateway，后置引入 | 内容规模和读者需求明确后再上。 |

核心判断：**让读文档这条主路径保持静态，让评论、搜索增强和未来 API 走明确的动态边界。**

## 框架判断

VuePress 可以部署到 Cloudflare，也适合写普通教程。实践型文档站更偏向 Astro + Starlight，因为它少维护主题、搜索和静态输出这些基础设施。
...
| 需要工具调用 | AI Search 或独立搜索接口 | 只有当搜索成为产品能力时再加。 |
