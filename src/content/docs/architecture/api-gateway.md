...
| 文件上传 API | Worker 鉴权 + R2 + D1 元数据 + 短期签名链接。 | 把 R2 访问凭证发给浏览器。 |
| 小型 SaaS API | Worker + D1 + KV 配置 + Workers Logs。 | 一开始拆成很多服务、很多数据库。 |
| 内部服务入口 | Worker + Access / Tunnel / 内部服务调用。 | 让内部服务直接暴露公网。 |
| 高成本 AI API | Worker + AI Gateway + 用户配额 + 缓存。 | 匿名无限调用模型。 |
| 客户或移动端 API | Worker + API Shield + 请求格式校验 + 更强身份校验。 | 没有请求格式、没有限流、没有观测。 |

## 三种形态

Cloudflare 官方 Use cases 把 API 方案拆成三类。不需要一开始全做，先看你在哪个阶段。

| 形态 | 什么时候用 | Cloudflare 组合 |
| --- | --- | --- |
| Edge-native APIs | 新项目直接跑在 Cloudflare 上。 | Workers、D1、KV、R2、Queues、Durable Objects。 |
| Secure API gateway | 已有源站或第三方 API，需要统一安全入口。 | WAF、Rate Limiting、API Shield、Workers、Access。 |
| Microservices mesh | 内部服务变多，需要不暴露公网地互相调用。 | 内部服务调用、Tunnel、Access、Workers VPC、Cloudflare One。 |

默认从 Edge-native APIs 起步。只有当你已有源站、第三方 API 或内部网络时，才需要补另外两类。
...
| 8 | API 面向客户、移动端或机器调用时，补 API Shield、Access 和更细的 Rate Limiting。 |
...
| 请求格式漂移 | 应用层校验请求；客户 API 再加 API Shield。 |
| 机器调用伪造 | 签名、服务凭证和更强身份校验。 |
| 内部服务暴露 | 内部服务调用或 Tunnel，不给内部服务公网入口。 |
