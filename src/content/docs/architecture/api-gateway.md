...
API 网关只解决一件事：把动态入口收清楚。Worker 负责鉴权、校验和路由；数据、文件、慢任务和强一致状态分别交给 D1、R2、Queues 和 Durable Objects。
...
| Webhook | Worker + 签名校验 + Queues + D1 状态表。 | Webhook 里同步做所有下游调用。 |
| 文件上传 | Worker 鉴权 + R2 + D1 元数据 + 短期签名链接。 | 把 R2 访问凭证发给浏览器。 |
...
| 顺序 | 做什么 |
| --- | --- |
| 1 | 先用一个 Worker 接住动态接口，只做路由、鉴权、输入校验和少量业务逻辑。 |
| 2 | 业务数据接 D1，文件接 R2，读多写少配置再接 KV。 |
| 3 | 邮件、导入、AI 后处理、Webhook retry 写入 Queues。 |
| 4 | 同一个资源有并发顺序问题时，再用 Durable Objects。 |
| 5 | API 面向客户、移动端或机器调用时，再补 API Shield、Access 和更细的 Rate Limiting。 |
