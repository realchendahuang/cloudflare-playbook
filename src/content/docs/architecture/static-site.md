...
| 需求 | 默认选择 | 不要先做什么 |
| --- | --- | --- |
| 纯文档站、官网、博客 | Pages 或 Workers Static Assets。 | 不要先做全站服务端渲染。 |
| 文档站 + 接口 / 评论 / 表单 | Workers Static Assets + 动态接口。 | 不要让所有静态页面先经过 Worker。 |
| 大量图片、附件、导出物 | R2 + 缓存策略。 | 不要把大文件塞进站点构建包或 D1。 |
...
| Pages | 纯静态文档、官网、博客、营销页、PR 预览优先的项目。 | Git 工作流轻，静态资产请求同样免费；动态函数进入 Workers 计费口径。 |

早期只写内容，Pages 很顺；如果同一仓库里还要放 Worker 接口、D1、R2、AI Search 或队列，Workers Static Assets 更适合作为长期入口。
...
| 搜索 | 搜索索引存在，线上搜索能命中文章。 |
| 动态路径 | 只有接口、评论、上传、后台等必要路径进入 Worker。 |
