...
静态内容站是最容易从 Cloudflare 获益的架构。文档站、官网、博客、作品集和知识库都应该先把阅读路径做成静态资产：页面、CSS、JS、图片和搜索索引直接从 Cloudflare 网络返回；评论、表单、搜索增强、文件权限和后台任务再交给 Worker。
...
| `/comments/*` | 成熟评论服务或独立评论 Worker。 | 评论数据进 D1，写入口要考虑验证和限流。 |
| `/files/*` | Worker + R2。 | Worker 做权限和签名，文件本体不要进静态站 bundle。 |
| `/admin/*` | Worker + Access / Tunnel。 | 后台必须先有身份边界。 |

整站先跑 Worker 只适合少数明确动态路径。套住整站阅读流量，会把本来免费的静态请求变成 Worker 调用。
...
| Worker CPU | SSR、AI 前处理、解析大文件和批处理要提前估算。 |
...
| 日志 | 记录路径、状态、耗时和请求编号，不记录密钥、登录凭证和正文隐私。 |
