...
观测要按“事实类型”分层，不要把所有数据都塞进同一个地方。

```text
用户体验
  └─ Web Analytics / Speed / Dashboard Analytics
...
这里的关键判断是：趋势图回答“整体有没有问题”，日志回答“这个请求发生了什么”，业务表回答“用户到底做了什么”，账单页面回答“是否会花钱”。这些问题看似相近，但事实源不一样。
...
普通开源项目不需要复杂观测平台，但正式上线前至少应该留住这几条线索：

| 项目 | 最小做法 | 为什么 |
| --- | --- | --- |
| 访问趋势 | 开 Web Analytics。 | 不写后端埋点，也能看页面访问和体验趋势。 |
| Worker 排障 | 开 Workers Logs，生产配置 `head_sampling_rate`。 | 避免高流量时日志量失控。 |
| 临时验证 | 用 Real-time logs 或 `wrangler tail` 看部署后请求。 | 快速确认新版本有没有异常。 |
| 单请求串联 | 在响应或日志里保留 request id / Ray ID。 | 出问题时能从浏览器、Worker、Security Events 串起来。 |
| 写接口保护 | 同时看 Workers Logs 和 WAF Security Events。 | 被边缘规则拦截的请求可能不会到达 Worker。 |
| 账单风险 | 配 Budget alerts 和 usage-based billing notification。 | 提醒不是封顶，但能尽早发现异常。 |
| 长期取证 | 只有合规、客户事故或安全调查需要时再上 Log Explorer / Logpush。 | 日志留存是成本，不是越多越好。 |

如果你的项目还停留在文档站、博客、开源教程和少量评论，默认组合就是 Web Analytics、Budget alerts、Workers Logs、Real-time logs。等 API、评论、AI 搜索或客户后台真的有使用量，再上 Analytics Engine 和长期日志。
...
Workers Logs 会收集 invocation logs、custom logs、errors 和 uncaught exceptions。新建 Worker 默认会启用 observability，但生产项目仍然建议把配置写进 `wrangler.jsonc`，避免 Dashboard 和代码仓库口径不一致。

```jsonc
{
  "$schema": "./node_modules/wrangler/config-schema.json",
  "observability": {
    // 开启 Workers Logs，让请求、console 日志和异常进入 Cloudflare 观测页。
    "enabled": true,
    // 生产高流量项目先采样，避免日志成本随着请求量线性失控。
    "head_sampling_rate": 0.1,
    "logs": {
      // invocation log 能补齐请求、响应和运行时元数据；极高流量时再评估关闭。
...
| 环境 | 建议采样 | 判断 |
| --- | --- | --- |
| 本地 / preview | `1` | 需要完整日志排查新改动。 |
| 低流量生产 | `1` | 请求量小于日志免费额度时，完整记录更省心。 |
| 中高流量生产 | `0.1` 或更低 | 先保留趋势和错误样本，再按事故临时提高。 |
| 高风险窗口 | 临时提高 | 发布、迁移、活动期可以短时间提高采样，结束后恢复。 |
...
| Logs Engine | 已经把 Logpush 日志放在 R2，想直接按时间或 Ray ID 查。 | 官方说明将被 Log Explorer 替代；新项目不要把它当长期主路线。 |
...
| 需求 | 推荐路线 | 不推荐 |
| --- | --- | --- |
| Worker 短期排障 | Workers Logs。 | 为 3 到 7 天问题直接上外部 SIEM。 |
| 部署后实时观察 | Real-time logs / `wrangler tail`。 | 把实时日志当长期存储。 |
| Workers Paid 项目长期留存 | Workers Trace Events Logpush 到 R2 或日志服务。 | 只开 Logpush 不设 health notification。 |
| Enterprise 多产品安全取证 | Log Explorer 或常规 Logpush。 | 只靠 Dashboard Analytics。 |
| Cloudflare 内部查询优先 | Log Explorer。 | 继续押注将被替代的 Logs Engine。 |
| 自己维护数据湖 | Logpush 到 R2，再接 Pipelines / 外部查询工具。 | 不估算日志体积就全字段全量推送。 |
...
| 字段 | 原因 | 替代做法 |
| --- | --- | --- |
| `authorization` / cookie | 凭证泄露风险最高。 | 只记录是否存在、认证方式和 hash 后的主体。 |
| Turnstile secret / API token | 这些值本来只该存在于服务端 secret。 | 记录 secret 名称或调用结果。 |
| 邮件正文 / 私信 / 评论全文 | 隐私和合规风险高。 | 记录内容长度、审核状态、错误类型。 |
| 完整查询字符串 | 可能包含 token、邮箱、搜索隐私。 | 只记录参数白名单或 query 类型。 |
| 大对象 / request body | 会冲击 256 KB 单条日志上限，也增加费用。 | 把对象存 R2，只在日志里记录 object key 和 size。 |

结构化日志的目标不是“记录一切”，而是让你能按 `event`、`request_id`、`ray_id`、`path`、`status`、`error_type` 快速过滤。真正的业务事实放 D1 / R2 / Analytics Engine，日志只保留排障所需的证据。
...
本站作为 Cloudflare 最佳实践仓库，观测路线也应该保持克制：

| 阶段 | 做什么 | 触发条件 |
| --- | --- | --- |
| 当前 | Web Analytics、Budget alerts、Worker deploy 后用 Real-time logs 验证。 | 静态文档页和评论服务仍是主场景。 |
| 评论增长 | 给评论 Worker 固化 Workers Logs、request id、Ray ID、错误类型和采样策略。 | 评论、审核、通知或反垃圾出现真实排障需求。 |
| 搜索增强 | Pagefind 仍走静态；AI Search / Worker 代理再加业务事件。 | 自然语言搜索上线后需要知道问题类型和失败率。 |
| 长期取证 | Workers Trace Events Logpush 到 R2 或 Log Explorer。 | 有安全事故、客户支持、合规或长期复盘要求。 |
| 产品指标 | Analytics Engine。 | 需要按页面、专题、搜索意图、评论互动做高基数指标。 |

这条路线的好处是：主站阅读路径仍然保持静态，只有评论、AI 搜索、后台任务这类动态能力进入日志成本。
