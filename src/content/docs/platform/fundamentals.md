---
title: Fundamentals
description: Cloudflare 账号、域名、代理状态、源站保护、权限和排障入口。
---

最后核对日期：2026-06-18。Fundamentals 是 Cloudflare 的底层边界：账号归谁、域名怎么接入、哪些请求真的经过 Cloudflare、源站能不能被绕过。

## 先记住

| 问题 | 判断 |
| --- | --- |
| 配置属于谁？ | User profile 是人，Account 是团队和资源，Zone 是域名。 |
...
域名接入细节放到 [DNS](/platform/dns/) 和 [SSL/TLS](/platform/ssl-tls/) 里看。这里先记住：Web 记录 Proxied，邮件和验证 DNS-only，目标 TLS 模式是 Full (strict)，排障时不要一上来 Pause Cloudflare。
