...
这些能力用于事务邮件、灰度发布、构建产物、代码执行和数据分析。它们不是普通项目的第一层，先确认真实需求和计费边界。
...
| 查询 R2 里的分析数据 | R2 SQL / R2 Data Catalog | 它们不是 D1 替代品。 |
| 让助手辅助配置和排障 | Agent Lee / Docs for agents | 生产变更仍要人工确认和留痕。 |

具体额度统一看 [免费额度大全](/platform/free-paid/)。会影响生产行为、发送成本、代码执行或数据扫描的能力，启用前先估量。
