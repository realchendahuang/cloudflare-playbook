...
Cloudflare DDoS Protection 官方说明所有计划都有 standard, unmetered DDoS protection；HTTP DDoS 和 Network-layer DDoS 都覆盖所有计划，但更细的 override、Log action、Adaptive / Advanced DDoS 和高级告警过滤属于更高计划或 Enterprise 能力。DDoS 的免费边界、源站保护、Under Attack 和误伤排查看 [DDoS Protection 精读](/platform/ddos/)。

WAF 也 available on all plans，但不同计划能力不同。WAF 的计划边界、Custom Rules、Managed Rules、Rate Limiting 和误伤排查看 [WAF 精读](/platform/waf/)。

普通项目先做这几件事：

1. 所有公开域名走 Cloudflare。
2. Web 入口记录保持 Proxied，源站防火墙只允许 Cloudflare IP 段和可信运维入口。
3. 登录、评论、上传、搜索 API 加限流。
4. 对后台路径加 Access 或 WAF 规则。
5. 日志里记录请求路径、状态码和必要上下文，不记录密钥。
6. 遇到攻击先保可用，再精细化规则。
