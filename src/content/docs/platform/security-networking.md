...
安全能力先按风险入口处理：公开网站保证入口正确，写接口加验证和限流，后台从公网收回来，密钥不要进代码，最后用日志和安全巡检复盘。
...
| API 被乱打 | 认证、限流、请求格式校验、最少日志。 | API 成为核心资产，再看更完整的 API 防护。 |
...
| Tunnel 发布后以为自动私有。 | 先建 Access 应用，再发布后台和内网工具。 |
| API Shield 当早期必选。 | 早期先做好认证、权限、请求格式和限流。 |
| Bot 问题只靠封 IP。 | 先识别路径和行为，再分层处理。 |
| 密钥写进配置或前端。 | Worker secrets；多项目共享再看 Secrets Store。 |

官方核对入口：[WAF](https://developers.cloudflare.com/waf/)、[DDoS Protection](https://developers.cloudflare.com/ddos-protection/)、[Turnstile](https://developers.cloudflare.com/turnstile/)、[Access 与 Tunnel](https://developers.cloudflare.com/cloudflare-one/access-controls/applications/http-apps/self-hosted-public-app/)。
