...
| 能力 | 用法 |
| --- | --- |
| DNS + Proxied | 域名统一托管，Web 入口优先橙云。 |
| Universal SSL / Full (strict) | 全站 HTTPS，源站也要有有效证书。 |
| DDoS Protection | 公开网站和 API 保持 Proxied。 |
| WAF | 保护登录、评论、上传、搜索、后台路径。 |
| Turnstile | 放在表单、评论、注册、登录和高频提交入口。 |
| Access / Tunnel | 后台、预览环境和内网工具先过 Access。 |
| Security Insights | 定期看配置风险和暴露入口。 |
