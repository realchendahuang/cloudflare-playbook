...
| 全站跳转、改写、挑战验证、缓存。 | 容易误伤证书验证、后台、API、登录和预览环境。 |
| 以为 Workers Paid 会提高规则配额。 | 规则跟域名或账号计划走，不跟 Workers Paid 走。 |
| 模拟通过就当线上没问题。 | 模拟只能排一部分问题；线上复盘还要看请求编号、真实日志和时间窗口。 |

## Page Rules 迁移

旧 Page Rules 不要机械搬。先问“这条规则想做什么”：跳转看跳转规则，缓存看缓存规则，回源看回源规则，改路径和头看改写规则。每次只动少量关键路径，观察后再删旧规则。
...
| 还在 Free 域名计划。 | Snippets 不可用，Workers Free 可以跑小 API。 |
...
| API 后端分流。 | 改写规则调路径，回源规则调后端；复杂鉴权交给 Worker。 |
