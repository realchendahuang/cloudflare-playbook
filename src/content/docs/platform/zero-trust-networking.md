...
普通项目优先看上半部分：Access、Tunnel、Gateway 和 Cloudflare One Client。Cloudflare WAN、Network Firewall、Magic Transit、Network Interconnect 更偏企业网络，不是个人文档站、小 SaaS 或单个管理后台的第一步。
...
| 模式 | 能力 | 适合 |
| --- | --- | --- |
| Traffic and DNS mode | DNS、Network、HTTP inspection、identity-based policies、device posture | 受管设备、完整 Zero Trust。 |
| DNS-only mode | 只过滤 DNS query，不做 HTTP inspection 和 device posture | 轻量起步、只想先拦域名。 |
...
| 企业网络 | 暂不使用 Cloudflare WAN / Network Firewall | 出现办公室、云网络、合规和专线需求后再评估。 |
