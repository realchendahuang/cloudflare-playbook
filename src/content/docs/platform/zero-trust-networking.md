...
| 场景 | 推荐方案 | 判断依据 |
| --- | --- | --- |
| 保护后台、预览环境、数据库面板 | Access + Tunnel | 后台不直接暴露公网，源站也不开放公网入口。 |
| 临时发布本地服务 | Tunnel public hostname | 快速发布；没有 Access 策略时就是公开访问。 |
...
| 3 | 机器访问使用服务凭证。 | 便于撤销和审计。 |
| 4 | 小团队从 DNS 出站过滤起步。 | 初期不需要配置全量客户端和 HTTPS 解密。 |
| 5 | 需要私网访问时再部署 Client。 | Client 是组织级动作，单站点通常用不到。 |
| 6 | 超出人数、日志或审计能力后再升级计划。 | 付费跟团队规模绑定。 |
...
| 企业网络能力 | 适用于办公室、数据中心、自有 IP 段或网络团队场景。 |
...
1. 公开站点优先完成 DNS、SSL/TLS、WAF、Turnstile。
