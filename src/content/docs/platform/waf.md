...
WAF 的作用不是“写越多规则越安全”，而是给网站入口和写接口加一层应用安全过滤。先保护后台、登录、评论、搜索、上传和写接口，静态阅读路径不需要复杂规则。
...
具体规则数量和计划边界回到 [免费额度大全](/platform/free-paid/) 核对。这里先记住：WAF 跟域名计划走，不跟 Workers Paid 走。

## 推荐顺序

1. 确认网站记录开启代理，源站不能被直连。
2. 打开基础托管规则，用安全事件观察真实命中。
