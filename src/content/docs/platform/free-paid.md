...
| 内存 | 128 MB。 | 128 MB。 |
| Subrequests | 50/request。 | 默认 10,000/request，可通过配置提高到更高限制。 |
| Worker 数量 | 100。 | 500。 |
...
| --- | --- | --- | --- |
| DNS | Free/Pro/Business 不对 DNS queries 收费，且不限制 DNS queries。 | Enterprise 把 monthly DNS queries 作为报价输入之一。 | 域名统一接入 Cloudflare，记录命名清楚，减少临时记录。 |
| CDN / Cache | 基础 CDN/cache 能力可用于 Free 计划。 | 更高级缓存控制、支持和企业能力随计划升级。 | HTML 谨慎缓存，静态资源长缓存；优先用 Cache Rules。 |
| SSL/TLS | Free 计划可提供基础 HTTPS。 | 高级证书、更多控制和企业安全要求再升级。 | 边缘和源站都保持 HTTPS。 |
| DDoS Protection | 官方说明所有计划都提供自动 DDoS 防护，并且 DDoS 防护是 unmetered。 | 需要更细策略、告警、支持和 SLA 时升级。 | 普通项目先把域名接入，出问题再结合 WAF、限流和日志。 |
| WAF | 官方说明 WAF available on all plans；Free Managed Ruleset 也 available on all plans。 | 托管规则、更大规则配额、Bot/API 安全能力随计划增强。 | 登录、后台、API 入口先加最小规则。 |
...
| Pages 静态站 | 静态资产请求免费且不限量；Free 计划 500 builds/month、1 concurrent build、100 custom domains/project、20,000 files/site、25 MiB/file。 | Pro/Business 提升构建次数、并发构建和文件数；Pages Functions 按 Workers 计费。 | 纯静态站可用 Pages；新 full-stack 项目优先 Workers Static Assets。 |
| Workers Static Assets | 静态资产请求免费且不限量。 | 只有请求进入 Worker 脚本时才按 Workers 计费。 | 文档站、官网、SPA 默认走静态资产；API 单独走 Worker。 |
...
| KV | 100k reads/day、1k writes/day、1k deletes/day、1k lists/day、1 GB storage。 | Paid 每月 10M reads、1M writes/deletes/lists、1 GB storage included，超出按量。 | 读多写少、最终一致的配置和缓存才用 KV。 |
| R2 | Standard storage 10 GB-month/month、Class A 1M/month、Class B 10M/month；egress 免费。 | Standard storage $0.015/GB-month，Class A $4.50/million，Class B $0.36/million。 | 大文件走 R2，不走 Git、Pages bundle 或 D1；下载次数也是 Class B。 |
| Hyperdrive | 100,000 database queries/day。 | Workers Paid 为 unlimited，仍要考虑外部数据库自身费用。 | 已有 Postgres/MySQL 再接；新小项目优先 D1。 |
| Queues | 10,000 operations/day included；消息保留 24 小时。 | Paid 1M operations/month included，超出 $0.40/million；保留期默认 4 天，可配到 14 天。 | 邮件、转码、同步、通知适合；简单同步接口不要先上队列。 |
| Durable Objects | Free/Paid 都可用；Free 只支持 SQLite-backed DO；100,000 requests/day、13,000 GB-s/day。 | Paid 1M requests/month included，超出 $0.15/million；400,000 GB-s/month included，超出 $12.50/million GB-s。 | 只放必须强一致的房间、会话、限流器；WebSocket 要用 hibernation。 |
| Durable Objects SQLite storage | Free 读 5M rows/day、写 100k rows/day、5 GB total。 | Paid 读写额度与 D1 类似，SQL stored data 5 GB-month included，超出 $0.20/GB-month。 | 新 DO class 优先 SQLite-backed；不要拿 DO 承担全站分析库。 |
