...
免费额度最值得用在“先把项目做出来”上，而不是把产品名堆满。下面这张表按普通人的项目目标整理，先判断能不能 0 元启动，再判断最可能撞到哪条线。
...
| 文件上传 / 下载 | R2 + signed URL + CDN cache | 附件、导出文件、私有下载、对象权限。 | storage、Class A、Class B。 |
...
| 实时小应用 | Durable Objects + WebSocket hibernation + Queues | 房间状态、实时连接、异步后处理。 | DO requests、GB-s、Queues operations。 |
