...
| 小团队后台 | Access + Tunnel | Zero Trust Free 50 个用户；后台入口走 Access 登录。 |
| AI 搜索原型 | Pagefind + AI Gateway + Workers AI / AI Search | 优先使用静态搜索；AI 调用要限流、缓存和记录用量。 |
| 实时小应用 | Durable Objects + Queues | DO 请求、运行时长、队列操作。 |

## 免费额度大全表

| 层级 / 产品 | 免费额度 / 免费限制 | 付费后变化 | 起步使用方式 |
| --- | --- | --- | --- |
...
| CDN / Cache | CDN 可用；Free 有 10 条缓存规则。 | 更多缓存规则和高级缓存能力跟域名计划走。 | 静态资源长缓存，HTML 短缓存或重新验证。 |
...
| Pages | Free 每月 500 次构建、1 个并发构建、每项目 100 个自定义域名、每站 20,000 个文件、单文件 25 MiB。 | Pro/Business 构建次数、并发和文件数更高；Pages Functions 按 Workers 计费。 | 纯静态站和 Git 预览部署。 |
...
| Vectorize | Free 每月 30M 查询向量维度、5M 存储向量维度。 | Paid 每月 50M 查询向量维度、10M 存储向量维度，超出按量。 | 文档少时先用 Pagefind；向量库只放明确搜索场景。 |
| Browser Run | Free 每天 10 分钟浏览器时间，Browser Sessions 3 个并发浏览器。 | Paid 每月 10 小时浏览器时间、10 个并发浏览器，超出按量。 | 能用普通请求完成时不使用浏览器；截图、PDF、动态页面抓取才评估。 |
...
| Containers | Free 不可用。 | Workers Paid 含 25 GiB-hours、375 vCPU-minutes、200 GB-hours，超出按量。 | 只有 Worker 运行时无法承载时再评估。 |
| 预算提醒 | 可按美元阈值发邮件提醒按量费用。 | 只发提醒，不自动停用产品。 | 停止超额靠限流、配额、队列熔断和产品开关。 |

## 5 美元 Workers Paid

| 能力 | Workers Paid 包含量 | 超额计费方式 | 适用说明 |
| --- | --- | --- | --- |
| Worker 请求 | 10M 次/月 | +$0.30 / 额外 1M 请求 | 评论、表单、接口、回调和搜索代理进入月度额度。 |
| CPU 时间 | 30M CPU ms/月 | +$0.02 / 额外 1M CPU ms | 服务端渲染、数据处理、签名上传、AI 代理和后台任务要看 CPU。 |
| 单次 CPU 上限 | 默认 30 秒，可提高到 5 分钟 | 仍需设置单请求上限 | 服务端渲染、数据处理和 AI 代理要单独看 CPU。 |
| Workers Logs | 20M 条日志事件/月，保留 7 天 | +$0.60 / 额外 1M 日志事件 | 日志留存从 3 天变 7 天。 |
| KV | 10M 读、1M 写、1M 删除、1M 列举/月，1 GB 存储 | 读写删列举和存储分别按量 | 配置、低频缓存和公开索引进入月度额度。 |
| D1 | 25B 行读、50M 行写、5 GB 存储/月 | 行读、行写、存储分别按量 | 评论、反馈、小后台和轻业务表进入月度行读写额度。 |
| Queues | 1M 操作/月，默认保留 4 天，可配置到 14 天 | +$0.40 / 额外 1M 操作 | 邮件、通知、导入和重试进入月度额度。 |
| Durable Objects | 1M 请求/月、400k GB-s/月 | 请求、运行时长和存储分别按量 | 房间、会话、限流器和实时状态进入月度额度。 |
...
| Containers | 25 GiB-hours、375 vCPU-minutes、200 GB-hours/月 | 内存、CPU、磁盘按秒计费 | Worker 运行时无法承载时再考虑。 |
...
| 企业网络能力 | Magic Transit、BYOIP、Network Interconnect、部分日志和 Bot 能力仍然是企业能力。 |
| 账单硬封顶 | 预算提醒只提醒，不会自动暂停服务或停止按量计费。 |

## Workers Paid 评估

只是文档站、官网、博客、作品集时，用 Free。静态资产请求免费且不限量，搜索用 Pagefind。小接口每天稳定几千到几万次时，继续看 CPU、日志和公开入口用量。

Worker 请求接近 100,000 次/天，或者 D1、KV、Queues、Durable Objects 已经是主要功能时，看 Workers Paid。需要 7 天日志、更多 Cron、更多子请求、更大包或更多 Worker，也看 Workers Paid。想加强 WAF、Bot、证书、缓存规则时，看域名计划。
...
| Hyperdrive、Workflows、Pipelines、Containers、R2 Data Catalog | 已有外部数据库、长流程、数据湖或完整运行环境限制。 |
...
- 所有请求都先交给 Worker 会浪费额度。静态资产直接返回，只让动态路径交给 Worker。
- KV 不能当数据库。KV 用于读多写少；关系数据用 D1。
- R2 出站带宽免费不代表完全免费。R2 不收出站带宽费，但存储和操作会计费。
- 不要过早使用向量搜索。先整理内容结构，Pagefind 能解决大部分早期搜索。
- Workers Paid 不等于 Cloudflare Pro。Workers Paid 是账号级开发者平台订阅；Pro / Business 是域名计划。
- 预算提醒不会自动封顶。它只发邮件，不暂停服务，也不停止按量计费。
- 日志不是越多越安全。日志要能定位问题，也要控制采样、留存和敏感字段。
