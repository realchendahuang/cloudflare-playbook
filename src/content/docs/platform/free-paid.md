...
| 小 API / Webhook | Workers Free + D1 / KV + 最小 WAF / 限流 | Workers 每天 100,000 次请求、10 ms CPU；KV 每天 1k 写入。 |
...
这张表把常用产品线摊开。读者不需要先懂每个产品名，只要先知道哪一层可以 0 元起步、哪一层会进入 `$5/month` Workers Paid、哪一层必须单独评估。
...
| WAF / 限流 | Free 有 5 条自定义规则、1 条限流规则和免费托管规则集。 | Pro / Business / Enterprise 提升规则数、字段、事件历史和高级检测。 | 登录、评论、上传、Webhook 先写最小规则。 |
...
| Workers 请求和 CPU | Free 100,000 次请求/天，10 ms CPU/次调用。 | Workers Paid 每账号最低 `$5/month`，含每月 1000 万次请求和 3000 万 CPU 毫秒。 | 只让必须运行代码的路径进入 Worker。 |
...
| Budget alerts | 按美元阈值发邮件提醒按量费用。 | 不是账单硬封顶，也不会自动暂停服务。 | 付费前必须开，但真正的控制来自静态优先、限流、配额和权限。 |
