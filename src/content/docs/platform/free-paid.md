...
| 预算提醒 | 可按美元阈值发邮件提醒按量费用。 | 仍然只是提醒，不是硬封顶。 | 必须开；硬边界靠限流、配额、队列熔断和产品开关。 |
...
把 5 美元/月理解成“开发者平台的生产底座”，而不是 Cloudflare Pro。它买的是动态执行、CPU、日志、队列、数据库、状态对象和部分后置能力的月度包含量。
...
| 想提升 WAF、Bot、证书、缓存规则 | 这不是 Workers Paid，要看域名计划。 |
