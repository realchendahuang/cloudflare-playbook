...
| 小 API、Webhook、评论、表单 | Workers Free 100,000 次动态请求/天，10 ms CPU/次调用。 | Workers + D1 + Turnstile + 最小 WAF / Rate Limiting。 | 请求量接近上限、CPU 不够、日志留存不够，或 D1 / KV / Queues 进入付费口径。 |
...
| 文件、图片、附件、导出物 | R2 Standard 10 GB-month、1M 写入类操作/月、10M 读取类操作/月，egress 免费。 | R2 + CDN cache。 | 存储、读写操作或冷数据归档开始超过免费层。 |
...
| 小 API / Webhook | Workers Free + D1 / KV + WAF Rate Limiting | 接口路由、鉴权、请求处理、数据写入。 | 100,000 次请求/天、10 ms CPU、写入滥用。 |
| 表单 / 反馈箱 | Workers + D1 + Turnstile + Workers Logs | 公开写入口、反垃圾、后台查看。 | D1 写入行数、日志 3 天留存。 |
| 文件上传 / 下载 | R2 + 签名链接 + CDN cache | 附件、导出文件、私有下载、对象权限。 | 存储量、写入类操作、读取类操作。 |
...
| 4 | 文件、附件、导出物进 R2。 | R2 免 egress，但要看存储量、写入类操作和读取类操作。 |
| 5 | 强一致状态进 Durable Objects，慢任务进 Queues。 | 房间、会话、限流器和后台任务不要塞进普通请求路径。 |
| 6 | 公开写入口加 Turnstile、WAF、Rate Limiting。 | 免费额度最怕被刷，安全边界要和功能一起上线。 |
...
| 存储产品要同时看容量和操作。 | R2 免 egress，但存储量、写入类操作和读取类操作会计费；D1 读的是扫描行数，KV 写入和列举很容易先撞限制。 |
...
| Workers 动态请求 | 100,000 次/天，UTC 0 点重置。 | 每账号每月最低 `$5`；含 10M 次请求/月，超出 `$0.30/million`。 | 评论、Webhook、小 API 免费可起步；公开 API 稳定有人用后再升。 |
| Workers CPU | 10 ms/次调用。 | 含 30M CPU ms/月，超出 `$0.02/million CPU ms`；单次默认 30s，可设到 5min。 | SSR、AI 前处理、批任务要看 CPU，不只看请求数。 |
...
| R2 Standard | 每月 10 GB-month、1M 写入类操作、10M 读取类操作，egress 免费。 | 超出按存储量、写入类操作和读取类操作计费；Infrequent Access 没有免费额度。 | 文件对象优先 R2，但公开下载热点要看读取类操作。 |
...
| --- | --- | --- |
| Worker 请求 | 100,000 次/天。 | 含 10M 次请求/月，超出 $0.30/million。 |
| CPU | 10 ms/次调用。 | 含 30M CPU milliseconds/月，超出 $0.02/million CPU ms；单次默认 30s，可设到 5min。 |
...
| Artifacts | 不可用。 | 含 10,000 次操作/月和 1 GB storage/month；超出 `$0.15/1,000 operations`、`$0.50/GB-mo`。 |
...
| [R2](/platform/r2/) | Standard storage 每月 10 GB-month、写入类操作 1M、读取类操作 10M；egress 免费；Infrequent Access 不包含免费额度。 | Standard storage $0.015/GB-month，写入类操作 $4.50/million，读取类操作 $0.36/million；Infrequent Access 为 $0.01/GB-month、写入类操作 $9.00/million、读取类操作 $0.90/million、data retrieval $0.01/GB。 | 大文件走 R2，不走 Git、Pages bundle 或 D1；下载次数也会计入读取类操作，冷数据再考虑 IA。 |
| [扩展计算与数据管道](/platform/extended-compute-data/) | Hyperdrive 100,000 次数据库查询/天；Workflows Free 与 Workers 共享 100,000 次请求/天、10 ms CPU/次调用、1 GB storage；Pipelines pricing 页列出 Free included usage；R2 Data Catalog 有 1M 目录操作/月、10 GB 压缩数据/月、1M 压缩对象/月。 | Hyperdrive Paid unlimited；Workflows Paid 含 10M 次请求/月、30M CPU ms/月、1 GB storage；Pipelines Paid transforms 50 GB/month、sinks 50 GB/month；Containers 仅 Workers Paid，包含 25 GiB-hours/month、375 vCPU-minutes/month、200 GB-hours/month；R2 Data Catalog Paid 超出后按目录操作和 compaction 计费。 | 有外部数据库、长流程、事件数据湖、容器运行时补位时再上；新项目先用 D1、R2、Queues、Workers。 |
...
| Stream | 没有适合普通文档站的免费存储额度；ingress 和 encoding 免费。 | 存储 $5/1,000 minutes stored，播放 $1/1,000 minutes delivered；Media Transformations 含 5,000 次免费操作/月，之后 $0.50/1,000。 | 视频产品用 Stream；普通文档不要把视频硬塞进静态站。 |
...
这张表只放最常被核对的官方入口。低频产品保留在对应平台页，不在这里堆成链接墙。
