...
最后核对日期：2026-06-18。Cloudflare 的额度和价格变化很快，本页用于建立判断框架，最终以官方 pricing / limits 页面为准。账单类型、预算提醒、发票和 usage-based billing 见 [Billing](/platform/billing/)。

这页是本站最重要的预算页。读 Cloudflare 产品时，先不要急着背产品名，先回答三个问题：免费额度能不能跑通真实项目，`$5/month` 的 Workers Paid 到底放大了什么，哪些能力必须走 zone plan、add-on 或 Enterprise。

## 本次核对方式

这页的数字不从二手博客摘录，按下面顺序核对：

| 顺序 | 来源 | 用途 |
| --- | --- | --- |
| 1 | Cloudflare 官方 pricing / limits 页面 | 确认免费额度、Workers Paid included usage、超额单价和限制。 |
| 2 | Cloudflare 官方 Markdown 版文档 | 用 `/index.md` 读取干净正文，减少 HTML 干扰。 |
| 3 | `cloudflare/cloudflare-docs` 源文件 | 追踪价格和限制表的原始 Markdown，后续方便看 diff。 |
| 4 | `cloudflare/templates` 等官方开源仓库 | 学真实项目如何组合 Workers、D1、R2、KV、Hono、Astro 和 Static Assets。 |

本页没有把 Budget alerts 写成“封顶机制”。Cloudflare 官方 Billing 文档把它定位为通知；真正的硬边界要靠架构、限流、队列、配额和权限控制。
...
Cloudflare 普通项目
  ├─ 免费且适合默认使用
  │    ├─ DNS / CDN / Universal SSL / DDoS
  │    ├─ Workers Static Assets 静态请求
  │    ├─ Pagefind 这类构建期搜索
  │    └─ 小规模 D1 / KV / R2 / Turnstile / Access
  ├─ $5 Workers Paid 解锁或放大
  │    ├─ Worker 动态请求与 CPU
  │    ├─ Pages Functions / KV / D1 / Queues / Durable Objects
  │    ├─ Workers Logs / Trace Events Logpush
  │    └─ Containers / Email Sending / Artifacts 等开发者平台能力
  └─ 不是 Workers Paid 能解决
       ├─ Zone 计划：Pro / Business / Enterprise
       ├─ Add-on：Images / Stream / Load Balancing / Argo 等
       └─ Enterprise：Magic Transit / BYOIP / Logpush / 高级 Bot / 完整 API Shield
```
...
下面这张表先解决“我做一个普通项目到底能免费用多少、$5 Workers Paid 又买到多少”的问题。它把普通人最常用的建站、API、数据库、文件、AI、安全和内网能力放在一起看；更细的媒体、日志、网络和企业产品边界继续看后面的长表。
...
| R2 Standard | 10 GB-month/month、1M Class A/month、10M Class B/month，egress 免费。 | 超出按 storage、Class A、Class B 计费；Infrequent Access 没有免费额度。 | 文件对象优先 R2，但公开下载热点要看 Class B。 |
...
| AI Gateway | 所有计划可用，核心功能免费；Persistent logs Free 为所有 gateway 合计 100,000 logs；Spend limits 可按模型、provider 或 metadata 做成本预算并返回 `429`。 | Workers Paid 为 10M logs/gateway；Logpush 仅 Workers Paid。 | 外部模型、Workers AI、缓存、限流、日志和成本控制统一从网关进。 |
...
普通开源项目、文档站、小工具和早期 SaaS，默认可以先用这些免费能力：

| 场景 | 推荐免费组合 | 需要注意的边界 |
| --- | --- | --- |
| 文档站 / 官网 | DNS、Universal SSL、CDN、Workers Static Assets、Pagefind。 | 静态请求免费；SSR、评论 API、AI 搜索等动态请求进入 Worker 计费。 |
| 评论 / 表单 | Worker Free、D1 Free、Turnstile Free、WAF 最小规则。 | Worker Free 是 100,000 requests/day；D1 Free 是每日读写限制，不是无限数据库。 |
| 小文件下载 | R2 Standard Free tier。 | R2 egress 免费，但 Class A / Class B 操作会计数；Infrequent Access 没有免费额度。 |
| 内部后台 | Cloudflare Access Free、Tunnel、Zero Trust Free。 | 50 user limit；没有 Access policy 的 Tunnel public hostname 仍然是公开入口。 |
| AI / 搜索试验 | Workers AI Free/Paid 每日免费 Neurons、AI Gateway core features、Pagefind。 | 高成本模型、持久日志、自然语言搜索和 Vectorize 都要单独估算。 |
| 安全巡检 | Security Insights Free、DDoS 默认防护、基础 WAF。 | Free 扫描频率是每 7 天；高级 Bot、完整 API Shield 和长期日志不是免费默认能力。 |

免费阶段的最佳实践不是“把所有东西都塞进免费额度”，而是让静态内容、动态写入、文件、数据库、日志和 AI 走各自正确的产品。这样未来升级 Workers Paid 时，成本增长会比较线性。
...
| [自有网络与专线](/platform/private-networking/) | Workers VPC 在 Open Beta 期间免费，Available on Free and Paid Workers plans；VPC Services 每账号 1,000 个。Magic Transit、BYOIP、Network Interconnect 都是 Enterprise-only。 | Workers VPC 仍按标准 Workers 请求和 CPU 计费；CNI ports 当前对 Enterprise customers no charge 但无正式 SLA；Magic Transit / BYOIP / CNI 需要合同和网络工程预算。 | 普通项目不要把自有 IP、专线和 L3/L4 防护当默认栈；Worker 访问私有服务时才评估 Workers VPC。 |
...
| [cloudflare/cloudflare-docs](https://github.com/cloudflare/cloudflare-docs) | Cloudflare 官方文档源仓库，价格、限制和 Markdown 文档源文件优先从这里追踪。 |
| [cloudflare/cloudflare-docs Workers pricing source](https://github.com/cloudflare/cloudflare-docs/blob/production/src/content/docs/workers/platform/pricing.mdx) | Workers pricing 源文件，适合核对 `$5 Workers Paid`、10M requests/month、30M CPU-ms/month、Workers Logs、KV、D1、Queues、Durable Objects、R2、Containers 等 included usage。 |
| [cloudflare/cloudflare-docs Workers limits source](https://github.com/cloudflare/cloudflare-docs/blob/production/src/content/docs/workers/platform/limits.mdx) | Workers limits 源文件，适合核对 CPU、subrequests、Worker 数量、Cron、Static Assets 文件数和 request body 计划边界。 |
| [cloudflare/cloudflare-docs Static Assets billing source](https://github.com/cloudflare/cloudflare-docs/blob/production/src/content/docs/workers/static-assets/billing-and-limitations.mdx) | Static Assets billing 源文件，适合核对“静态资产请求免费且不限量”和 `run_worker_first` 的免费层风险。 |
| [cloudflare/cloudflare-docs usage-based billing source](https://github.com/cloudflare/cloudflare-docs/blob/production/src/content/docs/billing/understand/usage-based-billing.mdx) | Usage-based billing 源文件，适合核对哪些产品按用量计费、included usage 和预算提醒口径。 |
| [cloudflare/cloudflare-docs D1 source](https://github.com/cloudflare/cloudflare-docs/tree/production/src/content/docs/d1) | D1 pricing / limits 源文件，适合核对 rows read、rows written、storage。 |
| [cloudflare/cloudflare-docs R2 source](https://github.com/cloudflare/cloudflare-docs/tree/production/src/content/docs/r2) | R2 pricing / limits 源文件，适合核对 storage、Class A、Class B、egress 和 Infrequent Access。 |
| [cloudflare/templates](https://github.com/cloudflare/templates) | Cloudflare 官方 Workers 模板集合，适合看免费优先项目如何组合 Astro、Vite、Hono、D1、R2、KV 和 Workers Static Assets。 |
| [cloudflare/skills](https://github.com/cloudflare/skills) | Cloudflare 官方 Agent Skills，适合把“先查官方、再写代码、再部署验证”的规则固化给 Codex。 |
| [cloudflare/mcp](https://github.com/cloudflare/mcp) | Cloudflare Code Mode MCP Server，适合后续让 Codex 通过 Cloudflare API 读取配置和执行平台操作。 |
| [twikoojs/twikoo-cloudflare](https://github.com/twikoojs/twikoo-cloudflare) | 本站评论服务参考，展示评论系统如何托管在 Workers 并使用 D1。 |
