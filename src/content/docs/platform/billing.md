...
Cloudflare 官方 Billing 页把一次付费订阅拆成这条链路：

```text
购买 plan / add-on
  └─ billing period 开始
      ├─ flat-rate: 为下一周期预付
      ├─ usage-based: 记录上一周期实际用量
      └─ invoice 生成
          ├─ payment success -> 服务继续
          └─ payment failed
              ├─ 5-day grace period
              ├─ 最多 5 次自动重试
              ├─ 可手动支付 outstanding balance
              └─ 仍失败 -> 降级到 Free，付费能力丢失
```

| 阶段 | 普通项目要做什么 |
| --- | --- |
| 第一次购买 | 记录购买日期、UTC billing date、购买人、产品和取消入口。 |
| 账期中 | 每周看 Billable Usage dashboard，异常产品回到架构里查请求路径。 |
| invoice 生成 | 区分 upcoming flat-rate charge 和 previous period usage-based charge。 |
| 支付失败 | 先处理付款方式和 outstanding balance；恢复后可能需要最多 24 小时。 |
| 被降级后 | 网站通常仍可访问，但付费功能会丢失；要逐个产品重新订阅。 |

Dashboard 里的 Billing 主要有四个入口：Invoices and documents 看发票和 invoice emails；Billable Usage 看按量费用；Payment 管付款方式、地址和税务信息；Subscriptions 看 active subscriptions、renewal date 和取消入口。
...
Cloudflare 官方的 How charges accrue 页给了一个很实用的判断：同一个页面请求可能同时碰到多个产品，每个产品按自己的维度计量。

```text
Proxied request
  ├─ DNS resolution: 普通 DNS query 不额外收费
  ├─ TLS / edge processing: 基础 SSL/TLS 随计划
  ├─ Cache hit: 通常是最便宜路径
  ├─ Cache miss
  │    ├─ Argo enabled -> Argo data transfer
  │    ├─ Worker route -> Worker request + CPU
  │    ├─ Worker reads KV / D1 / R2 -> 对应产品操作或行读写
  │    ├─ Origin fetch -> Cloudflare 不收普通 bandwidth egress
  │    └─ Cache Reserve enabled -> read / write / storage
  └─ Response delivered
```
...
这里的核心结论很朴素：**cache hit 是最便宜路径，静态资产不进 Worker 是最便宜架构**。一旦请求进入 Worker，再去读 R2、D1、KV、Images、AI 或外部 API，就要按每个产品分别看计量单位。
...
Billing 的风险不只在“月底账单变高”，还在“付款失败让付费能力突然消失”。
...
| stablecoin payments | Checkout 可用 USDC on Base / Polygon；支持一次性和 recurring billing。 | 它是支付方式，不是省钱工具；链上确认后不可 chargeback，仍要留备用付款方式。 |
...
| Payment methods | 支持常见信用卡、PayPal、Apple Pay、Google Pay、Stripe Link、UnionPay 等；部分 checkout 支持 USDC stablecoin payments。 |
