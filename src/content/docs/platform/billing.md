...
| 需要更多定时任务、Worker 数量、外部调用或更大构建包。 | 这是开发者平台工程化能力。 |
...
最低做法：启用付费前开预算提醒；每周看一次可计费用量；每个账期看发票。异常增长先查动态请求、CPU、R2 操作、媒体播放和 AI 调用。迁走流量、删除 DNS 或停止使用某个功能，不等于取消订阅；固定订阅仍要到账单订阅页处理。
