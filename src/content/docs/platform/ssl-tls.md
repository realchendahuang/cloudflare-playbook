---
title: SSL/TLS
description: Cloudflare SSL/TLS 的普通项目配置顺序、免费边界、Full (strict)、HSTS 和源站保护。
---

最后核对日期：2026-06-18。SSL/TLS 的计划权限、证书签发和浏览器兼容会变化；上线前以 Cloudflare SSL/TLS 官方页面为准。

SSL/TLS 只要先做对一件事：访客到 Cloudflare、Cloudflare 到源站，两段都加密。普通项目默认目标是 **Universal SSL + Full (strict)**。
...
| 想上传自有证书 | 普通项目通常不用；先看计划边界和真实兼容需求。 |

## 免费与计划边界

| 能力 | Free 是否够用 | 普通项目判断 |
| --- | --- | --- |
| Universal SSL | 通常够用 | 自动签发和续期，覆盖多数普通站点。 |
...
| Full | 可过渡，但不严格校验证书。 |
| Full (strict) | 普通项目默认目标。 |
| Strict (SSL-Only Origin Pull) | Enterprise-only，普通项目先不用。 |
...
普通项目先开 Always Use HTTPS，再用短 `max-age` 观察。不要一上来 preload。
