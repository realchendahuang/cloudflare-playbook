...
纯文档、官网、博客用 Pages 很合适；少量表单、Webhook 和轻 API 可以先用 Pages Functions。文件上传、附件和媒体放 R2；预览部署有敏感内容时加 Access；API、队列、Durable Objects、AI 组合变多后迁到 Workers Static Assets。

官方核对入口：[Cloudflare Pages](https://developers.cloudflare.com/pages/)、[Pages Limits](https://developers.cloudflare.com/pages/platform/limits/)、[Migrate from Pages to Workers](https://developers.cloudflare.com/workers/static-assets/migration-guides/migrate-from-pages/)。
