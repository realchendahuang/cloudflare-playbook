---
title: 流量调度与四层入口
description: Load Balancing、Health Checks、Spectrum 和 Argo Smart Routing 的普通项目取舍。
---
...
| Health Checks 能自动切流。 | 自动切流看 Load Balancing。 |
| Spectrum 可以保护所有端口。 | 只有配置了 Spectrum application 的端口才会代理到 origin。 |
| Argo 一开网站一定更快。 | 只有回源路径是瓶颈时才明显。 |
