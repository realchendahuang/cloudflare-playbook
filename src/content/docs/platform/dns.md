---
title: DNS
description: Cloudflare DNS 的普通项目接入顺序、免费边界、Proxy status、DNSSEC 和迁移清单。
---

最后核对日期：2026-06-18。DNS 的记录数、计划边界和高级 setup 会变化；上线前以 Cloudflare DNS 官方页面为准。

DNS 决定哪些流量进入 Cloudflare，哪些流量直接去第三方服务或源站。普通项目默认：**Full setup 接入域名，Web 入口 Proxied，邮件和验证记录 DNS-only，迁移前先处理 DNSSEC。**
...
| 需要 partial / secondary DNS 吗？ | 多数是企业场景，普通项目从 Full setup 起步。 |
...
| 2 | 在 Cloudflare 添加域名，让 quick scan 辅助导入。 |
| 3 | 手动核对 apex、`www`、API、后台、邮件、验证、CDN、对象存储和第三方服务。 |
| 4 | 对关键 DNS-only 记录降低 TTL。 |
| 5 | 处理旧 DNSSEC。 |
| 6 | 到注册商切 nameservers。 |
| 7 | 等 Zone Active 后验证网站、API、邮件、证书和第三方验证。 |
| 8 | 稳定后重新启用 DNSSEC，并清理旧 DNS provider 里的敏感记录。 |

## 只需要知道到这里

| 主题 | 普通项目判断 |
| --- | --- |
| TTL | Proxied 记录通常 Auto；迁移前只需要关心关键 DNS-only 记录。 |
| DNSSEC | 旧 DNSSEC 没处理好，切 nameserver 后可能解析失败。 |
| CNAME flattening | 解决根域名不能直接用 CNAME 的问题；第三方验证时要小心。 |
| Round-robin DNS | 不等于高可用；需要健康检查和故障切换时看 [流量调度](/platform/traffic-routing/)。 |
| Wildcard | 多租户或统一入口才用；普通项目精确记录更清楚。 |
...
| [Full setup](https://developers.cloudflare.com/dns/zone-setups/full-setup/) | 普通项目默认接入方式。 |
...
| [cloudflare/cloudflare-docs DNS source](https://github.com/cloudflare/cloudflare-docs/tree/production/src/content/docs/dns) | 官方 Markdown 源文件。 |
