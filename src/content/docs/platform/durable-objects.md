...
Durable Objects 的成本不能只看 requests。实时连接、RPC、alarm、SQLite 读写、对象 duration 和存储都会影响免费层。Free 的每日限制是硬边界，超过后后续操作可能失败，不是预算提醒。

| 边界 | 判断 |
| --- | --- |
| Requests | 请求、RPC、WebSocket message、alarm 都要看。 |
| Duration | 空闲连接要能 hibernate，长期常驻对象会放大成本。 |
...
| 单对象边界 | 按 room、user、tenant、resource 拆，不要全站一个对象。 |
