...
const emailPath = new URL('../docs/email.md', import.meta.url)
const emailSource = await readFile(emailPath, 'utf8')
const body = source
...
const emailBody = stripInlineIcons(emailSource)
  .replace(/^---\n[\s\S]*?\n---\n+/, '')
  .replace(/<script setup>[\s\S]*?<\/script>\n+/g, '')
  .replace(/<section class="onepage-hero">[\s\S]*?<\/section>\n+/g, '')
  .replace(/<div class="quick-grid">[\s\S]*?<\/div>\n+/g, '')
  // 子页原来的 ## 标题降级为 ###，嵌套在 "## 9. 邮件" 下面，保持 README 层级不冲突。
  .replace(/^## /gm, '### ')
  .replace(/^### /gm, '#### ')
  .replace(/^(#{3,4})\s+<[^>]+>\s+/gm, '$1 ')
  .replace(/[ \t]+\{#[^}]+}[ \t]*$/gm, '')
