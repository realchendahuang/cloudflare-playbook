...
<div class="quick-grid">
  <a href="#cloudflare-功能模块"><div class="card-icon"><Cloud /></div><div class="card-body"><strong>功能模块</strong><span>每个模块是什么、能干嘛、怎么选</span></div></a>
  <a href="#_2-ai-编程工作流"><div class="card-icon"><Bot /></div><div class="card-body"><strong>AI 编程工作流</strong><span>Skill、MCP、Wrangler 的配置方案</span></div></a>
...
进入 Codex 后输入 `/plugins`，搜索 `Cloudflare`，回车安装。完成配置。这一步会自动把 Cloudflare 的 Skill、MCP、集成自动完成安装，无需手动配置。
...
帮助 AI 理解 Cloudflare 怎么开发。Workers、Pages、R2、D1、KV、Queues、Durable Objects、Agents SDK、Wrangler、Email 这些它都会写。装上之后 AI 就不会把你当成在写普通 Node.js 项目了。

**MCP = 连接器**

让 AI 能连你的 Cloudflare 账号和官方文档。插件装好后下面这些会自动注册，不用自己一个个加：

- **Docs MCP**：使 AI 能够查询官方文档（日常必开）
...
Skill 和 MCP 都是"帮助 AI 理解"，真正要在你电脑上跑起来、部署上线，还得靠 Wrangler。在项目里装一下：
...
错误码不知道属于哪一层时，看第 5 节开头的错误码索引表。AI 看到 `522` 容易直接去改 Worker 代码，但 `522` 其实是源站问题——喂上下文的时候顺手告诉它错误码归属哪层，能省一大段弯路。

### 一句话总结

```text
插件先装  → 帮助 AI 理解 Cloudflare
Wrangler 跟上 → 使 AI 能够实际执行
Docs MCP 默认开 → 使 AI 能够查询文档
要动账号再开 API MCP → 使 AI 能够部署和修改配置
```
