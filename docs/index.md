...
<section class="onepage-hero">
  ...
  <p class="onepage-subtitle">用 AI 写代码，用 Cloudflare 免费部署到全球。涵盖功能模块、部署流程、免费额度与避坑指南。</p>
</section>
...
<div class="quick-grid">
  ...
  <a href="#_7-避坑指南"><div class="card-icon"><AlertTriangle /></div><div class="card-body"><strong>避坑指南</strong><span>官方文档提炼的工程约束与对策</span></div></a>
...
1. **为什么 Worker 跑着跑着就被杀了？**
   ...
   CPU 时间和 wall time 是两套账。CPU 只算代码执行时间，网络等待不计——Free 10ms / 请求，Paid 30s 可调到 5 分钟，超了返回 `1102`；wall time 按调用类型分，HTTP 请求无上限，Cron / Queue consumer / DO alarm 都是 15 分钟，超了强杀。重活（解析大 JSON、生成 PDF、处理图片）拆到 Queues 或 [Workflows](https://developers.cloudflare.com/workflows/)——后者每步独立持久化，单步失败只重试那一步，能跑几十分钟到几小时。

2. **为什么 `arrayBuffer()` 会让 Worker OOM？**

   128 MB 内存是 per-isolate，不是 per-request，并发请求共享这 128 MB。`await response.text()` / `arrayBuffer()` 把大 body 整个缓进内存，几个并发就撑爆；同时等响应头的出站连接最多 6 个（fetch、KV、R2、Queues 都算），响应头到了才释放名额。大 body 用流式透传 `return new Response(object.body, ...)`，不需要 body 就 `response.body.cancel()` 释放，并发 fetch 别一次开七八个。

3. **为什么部署被拒、冷启动慢？**

   Worker 压缩后 Free 3 MB / Paid 10 MB，启动时间必须 < 1 秒，否则部署直接被拒（错误 10021）。元凶通常是顶层重初始化——大 schema 解析、顶层 query DB。把配置、二进制、静态资产挪到 KV / R2 / D1 / Static Assets，用 Service Bindings 拆 Worker，顶层只做轻量初始化。

4. **为什么日志和缓存写入偶尔丢失？**

   没 `await`、没 `return`、没传给 `ctx.waitUntil()` 的 Promise 是浮动 Promise，运行时会在它完成前终结 isolate，结果静默丢失。症状是 webhook 偶发没发、缓存写入偶发失败、生产 bug 难复现。响应依赖结果就 `await`，不依赖就 `ctx.waitUntil()`；开 ESLint `no-floating-promises` 或 oxlint 对应规则在编译期拦住。

5. **为什么 A 用户的数据被下一个请求读到了？**

   Workers 复用 isolate 跨请求，模块级 `let` 变量在下一个请求里还在，会泄漏用户数据、状态错乱，甚至抛 `Cannot perform I/O on behalf of a different request`。binding 改动后 Cloudflare 可能复用旧 isolate，`let client ??= new Client(env.KEY)` 会拿到旧 secret。请求级数据走函数参数或 `env`，每请求 `new Client(env.MY_SECRET)`，要跨函数共享 binding 用 `import { env } from "cloudflare:workers"`（顶层不能做 I/O）。

6. **`ctx.waitUntil()` 有什么坑？**

   两个：一是解构丢 `this`，`const { waitUntil } = ctx` 运行时抛 `Illegal invocation`，必须 `ctx.waitUntil(...)`；二是响应发出后最多 30 秒，超时的后台任务被砍掉——要跑更久改用 Queues / Workflows。

7. **为什么 subrequest 比我代码里的 fetch 多？**

   subrequest = 任何 `fetch()` 或对 R2 / KV / D1 / Queues 的调用，Free 50 / 调用，Paid 10,000（可调到 10M）。重定向链每跳都计数，你以为发了一个 fetch，实际走了 5 跳就是 5 个 subrequest。跨 Worker 调用走 Service Bindings 而不是公网 fetch，省 subrequest。

8. **为什么 Workers Logs 里有的请求没日志？**

   单请求 log 数据上限 256 KB（含所有 `console.log` + 异常 + 请求元数据 + headers），超了该请求后续 log 全部丢弃。开 `observability.enabled` + `logs.head_sampling_rate`（1 全捕，高流量调低）+ `traces.enabled` + `traces.head_sampling_rate: 0.01`。用 `console.log(JSON.stringify({...}))` 结构化日志，才能搜索过滤。

9. **`passThroughOnException()` 能当错误处理用吗？**

   不能。它是 fail-open 机制——Worker 抛未捕获异常时把请求透传到 origin，迁移期有用，但会隐藏 bug、让调试变难。用显式 `try/catch` + 结构化错误响应，`console.error` 记日志后返回 500。
...
2. **D1 索引机制是什么？**
   ...
   D1 用 SQLite 引擎，索引在写入时自动维护，`INTEGER PRIMARY KEY` / `ROWID` 默认主键不需要额外索引。建索引后跑 `PRAGMA optimize`（内部跑 `ANALYZE`）让查询规划器生成最优计划。索引不能改只能删重建；写索引列多 1 行 rows_written，但几乎总是被 rows_read 节省抵消。

3. **为什么写查询失败了不重试？**

   D1 只对**只读**查询自动重试最多 2 次，**写查询遇瞬态错误（`Network connection lost`、`storage caused object to be reset`、`reset because its code was updated`）直接失败**。写操作自己写指数退避 + jitter，上限 5 次左右。

4. **开了读副本为什么读到旧值？**

   开启读副本后各区域自动建只读副本，但异步复制可能落后。直接 `env.DB.prepare()` 查可能读到旧值——"我明明提交了怎么没显示"。**必须用 Sessions API**：`env.DB.withSession("first-primary")` 强制首查走 primary（要最新数据时），`withSession("first-unconstrained")` 首查可走副本但 session 内保证 monotonic reads，`withSession(bookmark)` 从上次 session 继续。把 `session.getBookmark()` 写回响应头 `x-d1-bookmark`，下次请求带回。`meta.served_by_region` / `served_by_primary` 可观测走哪个副本。

5. **导入导出有什么限制？**

   `wrangler d1 execute --file` 单文件上限 5 GiB，更大需拆分。不能直接导出虚拟表（FTS5），需先删再导出再重建。JS Number 52 位精度——`int64` 大数会丢精度。`Statement too long` 错误：把 1000 行 INSERT 拆成多段 250 行。
...
1. **为什么 KV 写了新值，过一会儿还读到旧的？**
   ...
   KV 是最终一致边缘 KV，写入地点通常立刻可见，但**其他位置可能要等 60 秒或更久**才能读到新值。**负查找（key 不存在）也被缓存**——创建一个之前查过的 key 同样要等 60s。把 KV 当原子读写存储用必然出错。
...
3. **为什么 `cacheTtl` 反而让更新看不到？**
   ...
   `cacheTtl` 最小 30 秒，默认 60 秒。一旦某 key 在某区域以某 cacheTtl 被读，就缓存到该区域直到 ttl 过期或被驱逐，**不会因为别处写了新值而主动失效**。写频繁 + 要求很快看到更新的场景，cacheTtl 反而拖累。

4. **为什么读 1000 个 key 撞上限了？**

   单 invocation 对外部服务操作数 1000；bulk get 最多 100 keys；响应 size 上限 25 MB（超则 413）。单 Worker 调用读 > 1000 key 撞上限——用 bulk read（算 1 个操作）。把相关冷热 key 合并成单个"super key"对象让冷 key 跟热 key 一起缓存，但更新需加锁避免竞争。

5. **什么场景不该用 KV？**

   官方明确"write-heavy Redis 型"工作负载（同一 key 每秒写几十上百次）不适合 KV。余额、库存、订单状态、实时计数、任何需要原子读写的场景不要放 KV——用 D1 或 Durable Objects。KV 适合配置、用户偏好、白 / 黑名单、缓存、静态资产等读多写少场景。
...
1. **文件元数据和权限该怎么存？**
   ...
   R2 自带 `customMetadata` 但不适合结构化查询——"某用户最近 10 个文件"这种用 `list` 翻页做不了。文件 body 存 R2，元数据（R2 key、owner、created_at、size、contentType、权限）存 D1。查询走 D1（带索引、可分页排序），拿 key 后回 R2 取 body。

2. **为什么不要从 Worker 内用 REST API 调 R2？**

   官方反模式：从 Worker 内用 REST API (`api.cloudflare.com/.../r2/...`) 调自家 R2，多一跳认证多一跳网络。R2 binding 是进程内引用，零网络跳零认证零额外延迟。

3. **三种访问模式怎么选？**

   Worker binding 流式透传（要业务逻辑 / 鉴权，`return new Response(object.body, ...)`）、S3 API presigned URL（大文件客户端直传直读，省 Worker CPU 和 subrequest）、public bucket（纯公开静态资产，无鉴权）。大文件走 Worker 代理会吃 128 MB 内存和 CPU 时间，流式可缓解；走 presigned URL 要管签名过期和 key 泄露。

4. **`list()` 分页有什么坑？**

   R2 `put()` / `delete()` 是强一致——Promise resolve 后全球任意后续读都能看到。`list()` 返回最多 1000 条，可能更少以减压；用 `truncated` 判断是否还有，**不要用 `objects.length < limit` 判断**（会死循环）。未完成的 multipart upload 7 天后自动 abort，并发对同一 multipart upload 操作会冲突。

5. **怎么做条件读和范围读？**

   `get({ onlyIf: { etagMatches, uploadedBefore, uploadedAfter } })` 条件不满足时返回 `R2Object`（无 body），延迟更低。`range: { offset, length, suffix }` 支持范围读。可指定 `md5/sha1/sha256` 等校验（只能指定一种）。
...
1. **为什么所有请求塞进一个 DO 就成瓶颈？**
   ...
   单 DO 软上限约 1000 req/s（简单透传），JSON 解析 + 校验降到 500–750，带存储写 200–500，超了排队再 `overloaded`。把所有请求塞进 `idFromName("global")` 一个 DO 就是瓶颈。按"协调原子"建 DO——一房一 DO、一局一 DO、一用户一 DO。分片数 ≈ 总 req/s ÷ 单 DO 容量。

2. **新 DO 该用哪种存储后端？**

   官方明确所有新 DO class 应该用 SQLite backend（支持关系查询 / 索引 / 事务 / PITR），KV backend 仅 Paid 保留兼容。SQLite DO 存储上限 10 GB（Paid）/ 5 GB（Free 全账户），超了写抛 `SQLITE_FULL`，读和 delete 仍可。catch `SQLITE_FULL` 后删旧数据或返回错误；大文件存 R2，DO 只存元数据。每表 ≤ 100 列，行 / string / BLOB ≤ 2 MB，SQL 语句 ≤ 100 KB，绑定参数 ≤ 100 / 查询。
...
4. **为什么 `blockConcurrencyWhile()` 拖慢吞吐？**
   ...
   阻塞所有并发直到回调完成，每次 ~5ms 就把单 DO 限到 ~200 req/s。跨 I/O（fetch / KV / R2 / 外部 API）用 = 持锁跨 I/O，严重拖吞吐。只用于构造函数初始化 / migration；常规请求靠 input/output gate + write coalescing；跨 async 的原子读改写用 `transaction()`。

5. **确定性 Id 和 `newUniqueId` 怎么选？**

   `getByName(str)` 同输入永远同 ID（适合"某房间 / 某用户"路由）；`newUniqueId()` 生成随机 ID，**必须自己存映射**（如存 D1）才能再找到。创建 stub 不会实例化或唤醒 DO——只有调方法才激活。

6. **WebSocket 长连接怎么省费？**

   标准 `ws.accept()` 让 DO 一直占内存计费，空闲连接也烧 duration (GB-s)。`this.ctx.acceptWebSocket(ws)` 启用 Hibernation——DO 空闲可被驱逐出内存，WS 连接保持打开，来消息时自动唤醒重跑构造函数，hibernation 期间不计 duration 费。官方计费示例（100 DO × 50 WS × 1 msg/min × 8h/天 × 30 天）：标准 API ~$138/月，Hibernation ~$10/月。构造函数尽量轻（唤醒会重跑）；每连接状态用 `ws.serializeAttachment`（上限 16 KB，更大用 SQLite 存 key 再 attachment 存 key）；`setWebSocketAutoResponse` 让 ping/pong 不唤醒 DO。**出站 WebSocket 不支持 hibernation**，活跃出站连接让 DO 最多占内存 15 分钟。

7. **为什么大量小消息压垮 DO？**

   每条 WS 消息有 JS 运行时 ↔ 系统的 context switch 开销，大量小消息会压垮单个 DO，即便总数据量小。客户端把 10–100 条逻辑消息打包进一个 WS frame，高频数据按 50–100ms 或 50–100 条攒批。

8. **为什么 alarm handler 要写幂等？**

   每个 DO 可 `setAlarm(timestamp)` 安排未来任务（毫秒精度），`alarm()` handler 在无入站请求时也能跑。**alarm 偶发会触发多次**，handler 必须幂等。handler 抛异常会自动重试；重试快用完仍没完成会彻底失败。handler 内 catch 异常，若 `alarmInfo.retryCount >= 5` 就 `setAlarm(Date.now() + 30_000)` 自己续命；handler 开头 check 状态（如 `lastRenewal`）避免重复执行。**只在有活干时调度**——短间隔（秒级）唤醒所有 DO 会很贵。

9. **DO stub 抛异常还能继续用吗？**

   不能。异常带 `.retryable`（瞬态内部错，幂等请求可重试）和 `.overloaded`（DO 过载，**不该重试**，重试只会更过载）。`.remote = true` 表示异常来自远端 DO 代码或基础设施。**异常后 stub 进入"broken"状态，再用必失败**——每次尝试新建 stub；retryable 用指数退避 + jitter；overloaded 别重试，应减载或增加 sharding。

10. **删 DO 为什么不能只删 key？**

    DO 完全消失 = 关闭时存储为空。一旦写过任何 storage（含设过 alarm），单纯删 key / drop table 不够——内部元数据会残留，继续计存储费。必须 `storage.deleteAll()`；compat date < `2026-02-24` 还要先 `deleteAlarm()`。

11. **RPC 调用怎么计费？**

    compat date ≥ `2024-04-03` 应该用 RPC 而不是 `fetch()` handler——类型安全、免手动 request / response 解析。每个 RPC method call 是独立 RPC session = 1 次计费请求；但方法返回的 `RpcTarget` stub 上的后续调用算同一 session，不再计费。用 `DurableObjectNamespace<ChatRoom>` 类型参数让 stub 带类型；不要把 DO stub 缓存在全局变量（异常后 stub 会 broken）。
...
1. **为什么 10 条消息第 8 条失败，前 7 条也被重复处理？**
   ...
   Queues 是至少一次投递。默认**整个 batch** 在 `queue()` 返回且所有 `waitUntil` resolve 后才 ack；**只要 `queue()` 抛异常或任一 waitUntil reject，整个 batch 全部重投**。10 条消息第 8 条失败 → 1–7 条也被重复处理。

2. **非幂等操作怎么避免重复执行？**

   调外部 API、写 DB 这类非幂等操作**逐条 `msg.ack()`**——已 ack 的不会被重投；失败用 `msg.retry({ delaySeconds })` 单条重试。**`ack()` 优先级最高**：先 ack 后 retry，retry 生效；先 retry 后 ack，ack 被忽略。

3. **怎么对上游 429 做退避？**

   每条消息带 `attempts` 属性（从 1 开始），`delaySeconds` 0–86400。上游 429 用 `retry({ delaySeconds: base ** attempts })` 指数退避。可对单条 `msg.retry({ delaySeconds })` 或整批 `batch.retryAll({ delaySeconds })`，per-message 延迟优先于 queue 级。

4. **DLQ 和 `max_retries` 怎么配？**

   达 `max_retries`（默认 3，最大 100）后默认直接删除；配了 DLQ 则写入 DLQ。`retry()` / `retryAll()` 不会触发 consumer concurrency 缩容。非幂等 / 关键业务必配 DLQ 并监控长度。

5. **批处理大小怎么设？**

   `max_batch_size` 默认 10（1–100）；`max_batch_timeout` 默认 5s（0–60s），先到先触发。空队列不会推空 batch（push 消费者）；pull 消费者主动 pull 空队列会产生读操作计费。写外部系统用大 batch 省调用数，用户面活动相关用小 batch + 短 timeout 分散负载。

6. **handler 返回后还有活怎么办？**

   handler 返回后未 resolve 的 promise 可能不完成。`waitUntil()` 是唯一支持"handler 完成后再收尾"的机制。

7. **Queues 的关键限制是多少？**

   每队列吞吐 5000 msg/s（超则 `Too Many Requests`）；消息大小 128 KB（含 ~100B 内部元数据）；消息保留 Free 24h、Paid 可配最长 14 天；每队列 backlog 25 GB（超则 `Storage Limit Exceeded`）；并发 consumer 调用 250（push-based only）；consumer wall time 15 分钟、CPU 默认 30s 可调到 5 分钟。

8. **Queues 和 Workflows 怎么选？**

   Queues 是消息 broker，producer / consumer 解耦，适合 fan-out、缓冲、批处理、单步后台任务（发邮件、webhook、日志）。Workflows 是多步有依赖的 durable execution，每步返回值持久化，单步失败只重试该步，适合多步流程（扣款→发货→确认）、需暂停恢复（`step.waitForEvent()` 等数小时 / 天 / 人工审批）。组合：Queue 缓冲入口，consumer 为每条消息起一个 Workflow 实例做复杂处理。
...
1. **Worker 调 Worker 为什么不要走公网 fetch？**
   ...
   binding 是零成本、绕过公网、支持类型安全 RPC。从 Worker 内 fetch 同 zone Worker 的公网 URL **不带 service binding 会失败**；走公网 fetch 自家 Worker = 多余认证 + 多余网络跳 + 多余 subrequest + 没类型安全。被调 Worker `export class Foo extends WorkerEntrypoint { async bar() {...} }`，调用方 `env.FOO.bar()`（带类型）。

2. **Edge + Placed 模式怎么搭？**

   `auth-worker`（不开 placement，跑边缘做鉴权）→ Service Binding RPC → `app-worker`（开 Smart Placement 跑在 DB 附近）。每方法在 DB 附近跑，多次 round-trip 都低延迟。

3. **`WorkerEntrypoint` 的 `env` 怎么用？**

   类 entrypoint 上 `this.env.NAME` 可访问 binding；也可 `import { env } from "cloudflare:workers"` 顶层访问，但**顶层不能做 I/O**——`env.KV.get` 在顶层会错，`env.SECRET` 读值 OK。`withEnv({NAME:"Bob"}, () => ...)` 可临时覆盖用于测试。
...
1. **连外部 PG / MySQL 为什么必用 Hyperdrive？**
   ...
   在 DB 附近维护区域连接池，消除每请求 300–500ms 的 TCP + TLS + auth 开销，还能缓存查询结果。需 `nodejs_compat`。**每请求 `new Client({ connectionString: env.HYPERDRIVE.connectionString })`**——Hyperdrive 管底层池，client 创建很便宜；不要把 client 缓存到全局（binding 改动后 isolate 复用会拿到旧连接）。
...
1. **Smart Placement 影响哪些 handler？**
   ...
   自动分析流量把 fetch handler 跑在离 upstream 最近的 DC，**不影响 RPC methods / named entrypoints**；无 fetch handler 的 Worker 被忽略；Static assets 永远从离用户最近的 DC 发。部署后**最多 15 分钟**才有决策，需要"多个位置来的稳定流量"才能决策，否则返回 `INSUFFICIENT_INVOCATIONS`。所有请求带 `cf-placement` 头（如 `remote-LHR` / `local-EWR`）可验证。

2. **Placement Hints 怎么用？**

   知道后端在哪时直接指定，比 Smart 更可控：`placement.region = "aws:us-east-1"` / `"gcp:us-east4"` / `"azure:westeurope"`（CF 映射到离该云区域最近的 DC）、`placement.host = "db.example.com:5432"`（TCP CONNECT 探测，layer 4，实验性）、`placement.hostname = "api.example.com"`（HTTP HEAD 探测，layer 7）。Smart / region / host / hostname **互斥**，只选一个。单后端用 region hint 可把 RTT 从 20–30ms 降到 1–3ms；多后端 / 不知位置用 Smart。

3. **DO 自动 placement 怎么优化？**

   DO 查内嵌 SQLite 是零延迟（compute 和 data 同进程）。**尽量在 DO 内一次做完所有查询返回 composite result**，不要 Worker 多次 round-trip 调 DO。DO 创建位置默认是首请求附近，可 `get(id, { locationHint: "wnam" })` 给提示（建议非保证）。
...
2. **为什么要用 `wrangler types`？**
   ...
   手写 `Env` interface 会和实际 binding 漂移，部署时才发现不匹配。`npx wrangler types` 自动生成匹配 wrangler 配置的类型文件；加 / 改 binding 后重跑。

3. **`compatibility_date` 要怎么维护？**

   控制可用运行时功能和 bug fix。新项目设今天日期；存量项目定期更新拿到新 API 和修复。

4. **Custom Domain 和 Routes 有什么区别？**

   Custom Domain = Worker 就是 origin，CF 自动建 DNS + SSL；Routes = Worker 在已有 origin 前面，**必须先有 proxied（橙云）DNS 记录**，否则 `ERR_NAME_NOT_RESOLVED`。没真 origin 就加 proxied `AAAA` 指向 `100::` 占位。限制：Routes 1000 / zone、Custom Domains 100 / zone；`wrangler dev --remote` 时降到 50 / zone。

5. **为什么 Wrangler environments 的 binding 没继承？**

   `env.production` / `env.staging` 部署成 `name-production` / `name-staging`。**binding 和 vars 每 env 单独声明，不继承**；root Worker（不带 env 后缀）是独立部署，不打算用就别 `wrangler deploy` 不带 `--env`。
...
1. **为什么不能用 `Math.random()` 生成 token？**
   ...
   `Math.random()` 不是密码学安全。生成 token / ID 用 `crypto.randomUUID()` 或 `crypto.getRandomValues()`。`node:crypto` 在 `nodejs_compat` 下也完全支持。

2. **怎么防时序侧信道？**

   比较 secret 不要直接 `===`（会泄露长度信息）。用 `crypto.subtle.timingSafeEqual(a, b)`；先 SHA-256 hash 到定长再比较两个 hash 更稳，避免长度泄露。
