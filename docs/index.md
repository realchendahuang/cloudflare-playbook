...
1. **哪里找官方 cookbook 和模板？**
   ...
   官方没有独立的 cookbook 页，[Workers examples](https://developers.cloudflare.com/workers/examples/) 就是 cookbook 等价物，包含 SPA shell with bootstrap data、Stream large JSON、Sign requests、Stream OpenAI API Responses、Using timingSafeEqual、Turnstile with Workers、103 Early Hints、Cache Tags、Security headers 等示例。

   官方模板仓库 [cloudflare/templates](https://github.com/cloudflare/templates) 有：`d1-template`、`d1-starter-sessions-api-template`（D1 + Sessions API + 读副本 + 重试）、`durable-chat-template`（DO + WebSocket Hibernation）、`multiplayer-globe-template`（DO sharding）、`workflows-starter-template`、`postgres-hyperdrive-template`、`r2-explorer-template`、`react-router-hono-fullstack-template`（全栈 + RPC）。启动命令：`npm create cloudflare@latest`。

   来源：[Workers examples](https://developers.cloudflare.com/workers/examples/)、[cloudflare/templates](https://github.com/cloudflare/templates)
