...
<section class="onepage-hero">
  ...
  <p class="onepage-subtitle">用 AI 写代码，用 Cloudflare 免费部署到全球。涵盖功能模块、AI 编程工作流、免费额度与避坑指南。</p>
</section>
...
<div class="quick-grid">
  ...
  <a href="#_3-免费额度"><div class="card-icon"><Wallet /></div><div class="card-body"><strong>免费额度</strong><span>每个服务免费多少，超出怎么算</span></div></a>
  <a href="#_4-5-套餐"><div class="card-icon"><Zap /></div><div class="card-body"><strong>$5 套餐</strong><span>值不值得买，什么时候买</span></div></a>
  <a href="#_5-开源项目"><div class="card-icon"><Package /></div><div class="card-body"><strong>开源项目</strong><span>按用途整理可参考的项目</span></div></a>
  <a href="#_6-避坑指南"><div class="card-icon"><AlertTriangle /></div><div class="card-body"><strong>避坑指南</strong><span>官方文档提炼的工程约束与对策</span></div></a>
  <a href="#_7-国内访问"><div class="card-icon"><Globe /></div><div class="card-body"><strong>国内访问</strong><span>国内用户访问 CF 的注意事项</span></div></a>
...
AI 能自己开 Observability MCP 看日志、开 Browser Run MCP 看 Dashboard，但它不知道你这边看到的现象。让它排查线上问题前，把这些贴给它：

- 出问题的完整 URL。
- 发生时间和时区。
- HTTP 状态码（`522`、`1101` 这种）。
- 响应头里的 `cf-ray` 或 Ray ID——这是 Cloudflare 定位这次请求的唯一线索，AI 拿着它能在日志里精准找到这次请求的全程。
...
错误码不知道属于哪一层时，看第 6 节开头的错误码索引表。AI 看到 `522` 容易直接去改 Worker 代码，但 `522` 其实是源站问题——喂上下文的时候顺手告诉它错误码归属哪层，能省一大段弯路。
...
- 不要承诺“Cloudflare 一定能让国内访问很快”。
- 不要把生产业务只押在 `workers.dev`、`pages.dev` 或未验证的单线路访问上。
- 不要面向大陆正式运营还用未备案的国内服务器。
- 不要为了“加速”上灰色代理或境外 CDN 套壳绕备案，法律风险比技术风险大。
- 不要把第三方 JS 当必然可用，国内它会挂。
- 不要只在自己电脑上测通就上线，国内访问要按多地拨测来判断。
